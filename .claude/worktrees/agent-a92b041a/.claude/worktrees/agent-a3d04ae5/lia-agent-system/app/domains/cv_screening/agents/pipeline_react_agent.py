...
Implements the BaseAgent interface using a ReAct loop with pipeline-specific
tools, prompts and stage context. Manages candidate movement through the
recruitment Kanban pipeline. Follows the same pattern as WizardReActAgent.
"""
import logging
import time
...
class PipelineReActAgent(LangGraphReActBase, EnhancedAgentMixin):
    """Autonomous agent for the candidate recruitment pipeline.

    Implements the BaseAgent interface using a ReAct loop with
    pipeline-specific tools, prompts and stage context. Each call to
    ...
    def _get_system_prompt(self, input: AgentInput) -> str:
        """System prompt do Pipeline baseado no estágio atual."""
    ...
    async def _process_react_loop(self, input: AgentInput) -> AgentOutput:
        """Process a user message through the pipeline ReAct loop.
        ...
        logger.info(
            f"[PipelineReActAgent] Processing message for session={input.session_id} "
        ...
        try:
            ...
            tools = get_pipeline_tools(current_stage) + self._get_all_enhanced_tools()
            ...
            try:
                observer = ReActObserver(
                    session_id=input.session_id,
                    domain="pipeline",
                    agent_class="PipelineReActAgent",
                    company_id=input.company_id,
                    user_id=input.user_id,
                )
                ...
                logger.warning(f"[PipelineReActAgent] Failed to create observer: {obs_err}")
                observer = None
            ...
            logger.info(
                f"[PipelineReActAgent] Completed in {duration_ms:.0f}ms "
            ...
            try:
                ...
                logger.warning(f"[PipelineReActAgent] Failed to finalize observer: {obs_err}")
            ...
            duration_ms = (time.time() - start_time) * 1000
            logger.error(
                f"[PipelineReActAgent] Error processing message: {exc}",
                exc_info=True,
            )
            return AgentOutput(
                message=(
    ...
    async def _save_memory(
        ...
        try:
            ...
            if updates:
                ...
                logger.debug(
                    f"[PipelineReActAgent] Updated memory: {list(updates.keys())}"
                )
        except Exception as exc:
            logger.warning(
                f"[PipelineReActAgent] Failed to update memory: {exc}"
            )
    ...
    async def _extract_field_updates(
        ...
        for tc in state.tool_calls_made:
            ...
            if tool_name == "analyze_cv":
                updates["candidate_evaluated"] = True
                fit_score = result.get("data", {}).get("fit_score")
                if fit_score is not None:
                    updates["fit_score"] = fit_score

            elif tool_name == "run_wsi_screening":
                updates["screening_completed"] = True
                wsi_score = result.get("data", {}).get("wsi_score")
                if wsi_score is not None:
                    updates["wsi_score"] = wsi_score

            elif tool_name == "move_candidate":
                data = result.get("data", {})
                new_stage = data.get("new_stage")
                if new_stage:
                    updates["decision_made"] = True

            elif tool_name == "schedule_interview":
                updates["interview_scheduled"] = True
                data = result.get("data", {})
                updates["interview_type"] = data.get("interview_type", "")
                updates["interview_datetime"] = data.get("interview_datetime", "")

            elif tool_name == "generate_offer":
                updates["offer_decision"] = "sent"

            elif tool_name == "finalize_hiring":
    ...
    async def _check_navigation(
        ...
        if missing_required:
            logger.debug(
                f"[PipelineReActAgent] Cannot navigate: missing required fields "
        ...
        if not user_confirmed:
            logger.debug(
                f"[PipelineReActAgent] Required fields met for {current_stage} "
