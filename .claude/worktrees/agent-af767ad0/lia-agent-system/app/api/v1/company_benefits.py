...
from app.models.company_benefit import CompanyBenefit, DEFAULT_BRAZILIAN_BENEFITS
...
class CompanyBenefitCreate(BaseModel):
    ...
    value_type: Optional[str] = "informative"
...
async def seed_default_benefits(
    company_id: Optional[str] = Query(None, description="Company ID to seed benefits for"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user_or_demo)
):
    """
