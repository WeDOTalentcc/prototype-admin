...
${salaryInfo.minSalary && salaryInfo.maxSalary 
  ...
  const SectionCard = ({ 
    ...
    <div className="rounded-md border border-gray-200 overflow-hidden">
      <button
        ...
        className="w-full px-3 py-2 bg-[#F8FAFB] flex items-center justify-between hover:bg-[#F0F0F0] transition-colors"
      >
        <div className="flex items-center gap-2">
          ...
          {count !== undefined && (
            <span className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 text-[9px] font-medium rounded-full">
              {count}
            </span>
          )}
        </div>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-md border border-gray-200 overflow-hidden">
        <div className="px-3 py-2 bg-[#F8FAFB] flex items-center justify-between">
          <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 text-[10px] text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:bg-gray-800 rounded-full transition-colors"
          >
            {isGeneratingDescription ? (
      ...
      <SectionCard id="competencies" title="Competências" icon={Code} count={technicalSkills.length + enabledBehavioral.length}>
        <div className="space-y-2">
          <div>
            <span className="text-[10px] font-semibold text-gray-500 uppercase">Técnicas ({technicalSkills.length})</span>
            <div className="flex flex-wrap gap-1 mt-1">
              {technicalSkills.slice(0, 8).map(s => (
                <span key={s.id} className="px-1.5 py-0.5 bg-blue-50 text-blue-700 text-[9px] rounded-full">
                  {s.name}
                </span>
              ))}
          ...
          <div>
            <span className="text-[10px] font-semibold text-gray-500 uppercase">Comportamentais ({enabledBehavioral.length})</span>
            <div className="flex flex-wrap gap-1 mt-1">
              {enabledBehavioral.map(c => (
                <span key={c.id} className="px-1.5 py-0.5 bg-purple-50 text-purple-700 text-[9px] rounded-full">
                  {c.name}
                </span>
              ))}
            </div>
        ...
        <div className="space-y-1.5">
          {selectedWSI.map((q, i) => (
            <div key={q.id} className="flex items-start gap-2 text-[11px]">
              <span className="w-4 h-4 rounded-full bg-[#25D366]/10 text-[#25D366] text-[9px] flex items-center justify-center flex-shrink-0">
                {i + 1}
              </span>
      ...
      <div className="rounded-md border border-gray-200 overflow-hidden">
        <div className="px-3 py-2 bg-[#F8FAFB] flex items-center gap-2">
              ...
              <button
                ...
                className={cn(
                  ...
                  platform.enabled
                    ...
                    : "bg-[#F8F8F8] border border-transparent hover:border-gray-200"
                )}
              >
                <div className={cn(
                ...
                <span className="text-[11px] font-medium text-gray-800" style={{ fontFamily: '"Open Sans", sans-serif' }}>
                  {platform.name}
                </span>
              </button>
            ))}
