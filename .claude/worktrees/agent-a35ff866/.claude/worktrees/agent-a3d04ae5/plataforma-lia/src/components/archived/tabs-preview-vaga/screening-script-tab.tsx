// ARCHIVED: Tab Roteiro de Triagem - Removida em 15/01/2026 para simplificação do MVP
...
import { 
  ...
  MessageSquare, Globe, Phone, CheckCircle, CalendarCheck, Edit, 
  Plus, ChevronUp, ChevronDown, GraduationCap
} from "lucide-react"
...
interface ScreeningScriptTabProps {
  ...
  onShowWSITutorial?: () => void
  onEditQuestion?: (question: ScreeningQuestion | null) => void
}

export function ScreeningScriptTab({ previewJob, onShowWSITutorial, onEditQuestion }: ScreeningScriptTabProps) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <Button 
            size="sm" 
            variant="ghost" 
            className="h-5 gap-1 text-[9px] px-1.5 text-gray-500 hover:text-gray-900 dark:hover:text-gray-50 hover:bg-transparent"
            onClick={() => onShowWSITutorial?.()}
            title="Como funciona a metodologia WSI?"
          >
          ...
          <Button 
            size="sm" 
            variant="ghost" 
            className="h-5 gap-1 text-[9px] px-1.5 text-gray-600 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-transparent"
      ...
      <div className="p-3 bg-white border border-gray-100 rounded-md">
        ...
        <div className="grid grid-cols-4 gap-2 mt-2 pt-2 border-t border-gray-100">
          <div className="text-center">
            <div className="text-[13px] font-semibold text-gray-800">
              {Math.round(previewJob.funnel.screening * 0.6)}
            </div>
            <p className="text-[9px] text-gray-500">Triados</p>
          </div>
          <div className="text-center">
            <div className="text-[13px] font-semibold text-gray-800">
              {previewJob.funnel.total > 0 ? Math.round((previewJob.funnel.screening / previewJob.funnel.total) * 100) : 0}%
            </div>
          ...
          <div className="text-center">
            <div className="text-[13px] font-semibold text-gray-800">
              {previewJob.nps > 0 ? (previewJob.nps / 20).toFixed(1) : '4.2'}
            </div>
                        ...
                        {blockQuestions.length === 0 ? (
                          <div className="p-3 bg-gray-50 border border-gray-200 border-dashed rounded-md text-center">
                            ...
                            <Button 
                              size="sm" 
                              variant="ghost" 
                              className="mt-1.5 h-5 gap-1 text-[9px] px-2 text-gray-600 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200"
                              onClick={(e) => {
                                e.stopPropagation()
                                onEditQuestion?.(null)
                              }}
                          ...
                          blockQuestions.map((item: any, idx: number) => (
                            <div 
                              key={item.id || idx} 
                              className="p-2 bg-white border border-gray-200 rounded-md group hover:border-gray-900 dark:hover:border-gray-50/40 transition-colors"
                            >
                              <div className="flex items-start gap-2">
                                ...
                                >
                                  <Edit className="w-2.5 h-2.5 text-gray-400 hover:text-gray-900 dark:hover:text-gray-50" />
                                </Button>
                              </div>
                            </div>
                          ))
