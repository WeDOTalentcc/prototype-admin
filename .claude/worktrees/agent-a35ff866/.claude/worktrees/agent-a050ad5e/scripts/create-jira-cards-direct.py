#!/usr/bin/env python3
"""
Script para criar cards no Jira diretamente via API.
Usa a autenticação do Replit Connector.
"""
...
def get_jira_credentials():
    """Obtém credenciais do Jira via Replit Connector."""
    ...
    if not access_token or not site_url:
        raise ValueError('Could not get Jira credentials')
    
    return access_token, site_url


def create_jira_issue(access_token, site_url, project_key, summary, description, issue_type, labels, priority):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            try:
                ...
                error = {'status': response.status_code, 'text': response.text[:200] if response.text else 'Empty response'}
...
def main():
    ...
    try:
        access_token, site_url = get_jira_credentials()
        print(f"Conectado ao Jira: {site_url}")
    except Exception as e:
