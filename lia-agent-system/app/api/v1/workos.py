...
async def check_sso_domain(
    ...
    result = await db.execute(
        select(CompanyWorkOSConfig).where(
            CompanyWorkOSConfig.sso_enabled == True,
    ...
    result = await db.execute(
        select(ClientAccount).where(
            func.lower(ClientAccount.primary_email).like(f"%@{domain}")
        )
    )
...
async def sync_workos_user(
    ...
    """
    result = await db.execute(
        select(User).where(User.workos_id == user_data.workos_id)
    )
    ...
    if not user:
        ...
        if user:
            user.workos_id = user_data.workos_id
            user.workos_organization_id = user_data.organization_id
            user.sso_provider = user_data.connection_type
            user.last_sso_login_at = datetime.utcnow()
            user.updated_at = datetime.utcnow()
            logger.info(f"Linked existing user {user.email} to WorkOS ID {user_data.workos_id}")
        else:
            user = User(
        ...
        if user_data.organization_id:
            user.workos_organization_id = user_data.organization_id
        logger.info(f"SSO login for existing user: {user.email}")
...
async def scim_user_created(
    ...
    if user_data.directory_id:
        config_result = await db.execute(
            select(CompanyWorkOSConfig).where(
                CompanyWorkOSConfig.workos_directory_id == user_data.directory_id
            )
        )
        company_config = config_result.scalar_one_or_none()
        if company_config:
    ...
    if existing_by_email:
        existing_by_email.workos_id = user_data.workos_id
        existing_by_email.workos_directory_id = user_data.directory_id
        existing_by_email.is_scim_managed = True
        existing_by_email.updated_at = datetime.utcnow()
        if company_id and not existing_by_email.company_id:
            existing_by_email.company_id = company_id
        await db.commit()
        ...
        logger.info(f"SCIM linked existing user {user_data.email} to directory (company_id={company_id})")
        return SCIMActionResponse(
            success=True,
            action="created",
            workos_id=user_data.workos_id,
    ...
    logger.info(f"SCIM provisioned new user: {user_data.email} (company_id={company_id})")
    return SCIMActionResponse(
        success=True,
        action="created",
        workos_id=user_data.workos_id,
...
async def scim_user_updated(
    ...
    if user_data.state:
        user.is_active = user_data.state != "suspended"
...
async def scim_group_action(
    ...
    if action == "created":
        result = await db.execute(
            select(WorkOSGroup).where(WorkOSGroup.workos_id == group_data.workos_id)
        )
        existing = result.scalar_one_or_none()
        
        if existing:
            existing.name = group_data.name or existing.name
            existing.directory_id = group_data.directory_id or existing.directory_id
        ...
        if group_data.directory_id:
            group.directory_id = group_data.directory_id
...
async def scim_group_membership(
    ...
    result = await db.execute(
        select(User).where(User.workos_id == membership_data.user_id)
    )
    user = result.scalar_one_or_none()
    ...
    result = await db.execute(
        select(WorkOSGroup).where(WorkOSGroup.workos_id == membership_data.group_id)
    )
    group = result.scalar_one_or_none()
...
async def get_sso_status(
    ...
    scim_users_count = result.scalar_one()
    
    result = await db.execute(
        select(func.count(User.id)).where(
            User.company_id == company_id,
            User.workos_id.isnot(None)
        )
    )
    sso_users_count = result.scalar_one()
    
    groups_count = 0
    if config and config.workos_directory_id:
        result = await db.execute(
            select(func.count(WorkOSGroup.id)).where(
                WorkOSGroup.is_active == True,
                WorkOSGroup.directory_id == config.workos_directory_id
            )
        )
        groups_count = result.scalar_one()
    
    result = await db.execute(
        select(func.count(SSOAuditLog.id)).where(
            SSOAuditLog.company_id == company_id,
            SSOAuditLog.created_at >= datetime.utcnow() - timedelta(days=7)
        )
    )
    recent_events_count = result.scalar_one()
...
async def get_realtime_metrics(
    ...
    try:
        ...
        local_data["sso_users_count"] = sso_result.scalar() or 0
        ...
        local_data["scim_users_count"] = scim_result.scalar() or 0
        ...
        if config and config.workos_directory_id:
            ...
            local_data["groups_count"] = groups_result.scalar() or 0
    ...
    # Get organization_id for tenant-scoped API calls
...
async def get_groups(
    ...
    query = select(WorkOSGroup).where(
        WorkOSGroup.is_active == True,
        WorkOSGroup.directory_id == config.workos_directory_id
    )
...
async def set_group_role_mapping(
    ...
    result = await db.execute(
        select(WorkOSGroup).where(WorkOSGroup.id == uuid_module.UUID(group_id))
    )
    group = result.scalar_one_or_none()
    if not group:
    ...
    result = await db.execute(
        select(WorkOSGroupRoleMapping).where(
            WorkOSGroupRoleMapping.company_id == company_id,
            WorkOSGroupRoleMapping.workos_group_id == uuid_module.UUID(group_id)
        )
    )
    existing = result.scalar_one_or_none()
    ...
    if existing:
        ...
        existing.permissions = mapping.permissions
    else:
        new_mapping = WorkOSGroupRoleMapping(
            company_id=company_id,
...
async def get_audit_logs(
    ...
    """
    query = select(SSOAuditLog).where(
        SSOAuditLog.company_id == company_id
...
async def get_sso_users(
    ...
    if scim_only:
        query = query.where(User.is_scim_managed == True)
...
async def handle_dsync_user_created(
    ...
    if existing_by_email:
        existing_by_email.workos_id = workos_id
        existing_by_email.workos_directory_id = directory_id
        existing_by_email.is_scim_managed = True
        existing_by_email.company_id = company_id
        existing_by_email.updated_at = datetime.utcnow()
        await db.commit()
...
async def handle_dsync_user_updated(
    ...
    if state:
        user.is_active = state != "suspended"
...
async def handle_dsync_group_created(
    ...
    if existing:
        ...
        existing.updated_at = datetime.utcnow()
        await db.commit()
        logger.info(f"SCIM group already exists, updated: {workos_id}")
        return {"success": True, "message": "Group already exists, updated", "group_id": str(existing.id)}
    ...
    logger.info(f"SCIM created group: {workos_id} ({name})")
    return {"success": True, "message": "Group created successfully", "group_id": str(group.id)}
...
async def handle_dsync_group_updated(
    ...
    if name:
        group.name = name
    if directory_id:
        group.directory_id = directory_id
    if raw_attributes:
        group.raw_attributes = raw_attributes
    group.updated_at = datetime.utcnow()
    await db.commit()
...
async def handle_dsync_group_user_added(
    ...
    user_workos_id = user_data.get("id")
...
SCIM_EVENT_HANDLERS = {
    "dsync.user.created": handle_dsync_user_created,
    "dsync.user.updated": handle_dsync_user_updated,
    "dsync.user.deleted": handle_dsync_user_deleted,
    "dsync.group.created": handle_dsync_group_created,
    "dsync.group.updated": handle_dsync_group_updated,
    "dsync.group.deleted": handle_dsync_group_deleted,
    "dsync.group.user_added": handle_dsync_group_user_added,
    "dsync.group.user_removed": handle_dsync_group_user_removed,
}
...
async def scim_webhook(
    ...
    handler = SCIM_EVENT_HANDLERS.get(event_type)
    handler_result = {"success": True, "message": f"Event {event_type} acknowledged"}
    
    if handler:
        try:
            handler_result = await handler(data, directory_id, company_id, event_id, db)
        except Exception as e:
    ...
    if event_type.startswith("dsync.user."):
        actor_id = data.get("id")
        actor_email = data.get("emails", [{}])[0].get("value") if data.get("emails") else None
        target_id = data.get("id")
        target_email = actor_email
    elif event_type.startswith("dsync.group.user_"):
        user_data = data.get("user", {})
        group_data = data.get("group", {})
        actor_id = user_data.get("id")
        actor_email = user_data.get("emails", [{}])[0].get("value") if user_data.get("emails") else None
        target_id = group_data.get("id")
    elif event_type.startswith("dsync.group."):
        actor_id = data.get("id")
        target_id = data.get("id")
