...
# ============================================================================
# RE-ENRICH DISCARDED CANDIDATE — Task #402
# ============================================================================


class EnrichDiscardedRequest(BaseModel):
    """Request para re-tentar enriquecimento de um candidato descartado.

    Task #402: o usuário viu um perfil descartado pelo backend (sem email/
    telefone após enriquecimento) e pediu uma nova tentativa via Apify.
    """
    linkedin_url: str = Field(..., description="URL do LinkedIn do perfil")
    candidate_id: str | None = Field(
        None, description="UUID do candidato local, se existir"
    )
    candidate_name: str | None = Field(None, description="Nome (telemetria)")


class EnrichDiscardedResponse(BaseModel):
    """Resposta do re-enriquecimento por LinkedIn URL."""
    success: bool
...
@router.post("/enrich-discarded", response_model=EnrichDiscardedResponse)
async def enrich_discarded_candidate(
    request: EnrichDiscardedRequest,
    db: AsyncSession = Depends(get_db),
):
    """
    Task #402: força uma nova tentativa de enriquecimento Apify para um
    candidato que havia sido descartado por falta de email/telefone.

    Usa `enrich_candidate_contact(force=True)` quando temos UUID local;
    caso contrário, recorre a `enrich_by_linkedin_url` (URL-only scrape).
    """
    if not request.linkedin_url:
        raise HTTPException(status_code=400, detail="linkedin_url é obrigatório")
    ...
    try:
        if cand_uuid:
            try:
                result = await enrichment_svc.enrich_candidate_contact(
                    db=db,
                    candidate_id=cand_uuid,
                ...
                if result.get("success"):
                    has_contact = bool(result.get("has_contact"))
                    return EnrichDiscardedResponse(
                        success=True,
                        ...
                        message=(
                            "Contato enriquecido via Apify"
                            if has_contact
                            else "Apify executou mas não retornou email/telefone"
                        ),
                    )
                _contact_logger.info(
                    "[EnrichDiscarded] DB-backed enrichment failed for %s: %s",
                    cand_uuid,
                    result.get("error"),
                )
            except Exception as inner_err:
                _contact_logger.warning(
                    "[EnrichDiscarded] DB-backed enrichment raised, falling back to URL-only: %s",
                    inner_err,
                )

        url_result = await enrichment_svc.enrich_by_linkedin_url(request.linkedin_url)
        has_contact = bool(url_result.get("has_contact"))
        success = bool(url_result.get("success"))
        message: str
        if has_contact:
            message = "Contato enriquecido via Apify"
        elif success:
            message = "Apify executou mas não retornou email/telefone"
        else:
            message = url_result.get("error") or "Falha ao executar enriquecimento"
        return EnrichDiscardedResponse(
            success=success,
            has_contact=has_contact,
            email=url_result.get("email"),
            phone=url_result.get("phone"),
            linkedin_url=request.linkedin_url,
            candidate_id=str(cand_uuid) if cand_uuid else None,
            message=message,
        )
    except HTTPException:
        raise
    except Exception as e:
        _contact_logger.exception("[EnrichDiscarded] Unexpected failure: %s", e)
        raise HTTPException(
            status_code=500,
            detail=f"Falha ao re-enriquecer: {str(e)}",
        )
