"""
Pipeline Stage Templates — canonical CRUD endpoints.

Audit 2026-05-20 Sprint 2 (catalogos dinamicos):
substitui o catalogo hardcoded DEFAULT_STAGES (RecruitmentJourneyConfig.tsx +
...
from app.schemas.pipeline_stage_template import (
    CustomizeMasterRequest,
    PipelineStageTemplateCreate,
    PipelineStageTemplateListResponse,
    PipelineStageTemplateResponse,
...
router = APIRouter(
    prefix="/pipeline-stage-templates",
...
@router.get(
    "",
    response_model=PipelineStageTemplateListResponse,
    summary="List pipeline stage templates (master + custom da company)",
)
async def list_templates(
    ...
    return PipelineStageTemplateListResponse(
        items=items,
        total=len(items),
...
@router.get(
    "/{template_id}",
    response_model=PipelineStageTemplateResponse,
...
async def create_template(
    ...
    try:
        ...
        await db.commit()
        logger.info(
            "Pipeline stage template created: id=%s company=%s by=%s",
            template.id,
            company_id,
...
async def delete_template(
    ...
    await db.commit()
    logger.info(
        "Pipeline stage template soft-deleted: id=%s by_admin=%s",
        template_id,
        current_user.id if current_user else "unknown",
    )
    return None
...
async def customize_master(
    ...
    try:
        ...
        await db.commit()
        logger.info(
            "Pipeline stage master customized: master=%s -> custom=%s company=%s",
            master_id,
            custom.id,
            company_id,
        )
        ...
        await db.rollback()
        logger.exception("Failed to customize master pipeline stage template")
