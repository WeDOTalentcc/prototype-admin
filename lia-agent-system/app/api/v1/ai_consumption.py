...
async def get_or_create_balance(db: AsyncSession, company_id: UUID) -> AiCreditsBalance:
    """Get or create balance record for company."""
    query = select(AiCreditsBalance).where(AiCreditsBalance.company_id == company_id)
...
async def _get_usage_summary_data(company_id: str, db: AsyncSession) -> UsageSummaryResponse:
    """Internal helper to get usage summary data."""
