...
async def get_public_shared_search(
    ...
    if is_authenticated:
        ...
        candidates = build_candidate_snapshots(all_candidates, limit=5)
...
async def request_otp(
    ...
    try:
        ...
        if r is not None:
            allowed, _count = await _otp_rate_limiter._redis_sliding_window(rl_key, 5, 900)
            if not allowed:
                raise HTTPException(
                    status_code=429,
...
async def verify_otp(
    ...
    - Rate limited: 10 attempts per 15 minutes per token (anti-brute-force OTP)
    """
    # Rate limit: 10 verify attempts per 15 minutes per token
    rl_key = f"otp_rl:verify:{token}"
    try:
        r = await _otp_rate_limiter._get_redis()
        if r is not None:
            allowed, _count = await _otp_rate_limiter._redis_sliding_window(rl_key, 10, 900)
            if not allowed:
                raise HTTPException(
                    status_code=429,
