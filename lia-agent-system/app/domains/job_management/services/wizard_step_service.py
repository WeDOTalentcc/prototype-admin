    ...
    async def process(
        ...
        try:
            ...
            if company_profile and company_profile.additional_data:
                ...
                # Apply work_model default if not detected
                ...
                # Apply location default from company headquarters if not detected
            ...
            historical_patterns = {}
            if not job_draft.get('work_model') and not job_draft.get('modeloTrabalho'):
                # Company default not available, fetch historical pattern
                try:
                    ...
                    if 'work_model' in historical_patterns:
                        ...
                        job_draft['work_model_suggestion_context'] = f"Baseado em {pattern['percentage']}% das suas {pattern['count']} vagas anteriores"
                        field_origins['work_model'] = {
                            'source': 'historical_pattern',
            ...
            if not job_draft.get('employment_type') and not job_draft.get('tipoContrato'):
                try:
                    ...
                    if 'employment_type' in historical_patterns:
                        ...
                        job_draft['employment_type_suggestion_context'] = f"Baseado em {pattern['percentage']}% das suas vagas anteriores"
            ...
            if not job_draft.get('location') and not job_draft.get('localizacao'):
                try:
                    ...
                    if 'location' in historical_patterns:
                        ...
                        job_draft['location_suggestion_context'] = f"Baseado em {pattern['percentage']}% das suas vagas anteriores"
                        field_origins['location'] = {
                            'source': 'historical_pattern',
                            'confidence': min(0.85, 0.5 + (pattern['percentage'] / 200)),
                            'action': 'suggest',
