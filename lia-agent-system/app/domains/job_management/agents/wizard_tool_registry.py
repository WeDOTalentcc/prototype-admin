...
# ─── create_job_draft — NEW TOOL (eval fix: WZ-001) ─────────────────────────
@tool_handler("wizard")
async def _wrap_create_job_draft(**kwargs: Any) -> dict[str, Any]:
    """Create a brand-new job draft record and return the draft_id for subsequent saves."""
    raw_input: str = kwargs.get("raw_input", "") or kwargs.get("title", "") or ""
    job_title: str = kwargs.get("job_title", "") or kwargs.get("title", "")
    seniority: str = kwargs.get("seniority", "")
    work_model: str = kwargs.get("work_model", "")
    location: str = kwargs.get("location", "")
    detected_skills: list = kwargs.get("detected_skills", []) or []
    detected_responsibilities: list = kwargs.get("detected_responsibilities", []) or []

    # Auto-inject company_id from JWT context
    company_id: str = kwargs.get("company_id", "") or ""
    recruiter_id: str = kwargs.get("recruiter_id", "") or ""
    if not company_id:
        try:
            from app.shared.tenant_llm_context import get_current_llm_tenant
            company_id = get_current_llm_tenant() or ""
        except Exception as _e:
            logger.debug(f"[wizard_tools] create_job_draft: could not auto-inject company_id: {_e}")

    if not company_id:
        return {
            "success": False,
            "error": "company_id não identificado. Não é possível criar rascunho sem empresa associada.",
        }

    logger.info(f"[wizard_tools] create_job_draft: title={job_title!r} company={company_id!r}")
    try:
        ...
        async with AsyncSessionLocal() as session:
            await session.execute(sql_text("""
                INSERT INTO job_drafts (
                    id, company_id, recruiter_id, raw_input, job_title,
                    seniority, work_model, location, skills,
                    status, current_step, created_at, updated_at
                ) VALUES (
                    :id, :cid, :rid, :raw, :title,
                    :sen, :wm, :loc, :skills,
                    draft, input, NOW(), NOW()
                )
            """), {
                "id": new_id,
                "cid": company_id,
                "rid": recruiter_id or "lia_wizard",
                "raw": raw_input or job_title,
                "title": job_title or None,
                "sen": seniority or None,
                "wm": work_model or None,
                "loc": location or None,
                "skills": detected_skills,
            })
            await session.commit()

        title_display = job_title or "Nova Vaga"
        skills_display = ", ".join(detected_skills[:5]) if detected_skills else "a definir"
        model_display = work_model or "a definir"
        return {
            "success": True,
            "draft_id": str(new_id),
            "job_title": title_display,
            "work_model": model_display,
            "detected_skills": detected_skills,
            "detected_responsibilities": detected_responsibilities,
            "message": (
                f"Rascunho criado com sucesso para a vaga de **{title_display}**. "
                f"Skills identificadas: {skills_display}. "
                f"Modalidade: {model_display}. "
                f"draft_id={new_id} — pronto para enriquecimento e aprovação."
            ),
        }
    except Exception as exc:
        logger.error(f"[wizard_tools] create_job_draft error: {exc}", exc_info=True)
        return {
            "success": False,
            "error": f"Erro ao criar rascunho: {str(exc)[:200]}",
        }
...
TOOL_DEFINITIONS.append(
    ToolDefinition(
        ...
        description=(
            "Cria um novo rascunho de vaga no banco de dados. "
            "Use SEMPRE que o usuario pedir para criar uma nova vaga. "
            "Extrai os requisitos do input e retorna o draft_id para salvar dados adicionais. "
            "IMPORTANTE: company_id e recruiter_id sao auto-injetados da sessao JWT — nao peca ao usuario."
        ),
        parameters={
            "type": "object",
            "properties": {
                "job_title": {"type": "string", "description": "Titulo do cargo (ex: DevOps Engineer Senior)"},
                "raw_input": {"type": "string", "description": "Texto original do usuario descrevendo a vaga"},
                "seniority": {"type": "string", "description": "Nivel de senioridade (Junior, Pleno, Senior, Especialista)"},
                "work_model": {"type": "string", "description": "Modalidade de trabalho: remoto, hibrido, presencial"},
                "location": {"type": "string", "description": "Localizacao/cidade"},
                "detected_skills": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Skills tecnicas detectadas no input (ex: Kubernetes, AWS, CI/CD)",
                },
                "detected_responsibilities": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Responsabilidades detectadas no input",
                },
                "company_id": {"type": "string", "description": "ID da empresa (OPCIONAL — auto-injetado da sessao JWT)"},
                "recruiter_id": {"type": "string", "description": "ID do recrutador (OPCIONAL — auto-injetado)"},
            },
            "required": ["job_title"],
        },
...
# Also add create_job_draft to input-evaluation stage
STAGE_TOOLS["input-evaluation"] = ["create_job_draft"] + STAGE_TOOLS.get("input-evaluation", [])
