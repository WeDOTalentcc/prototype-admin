...
async def run(params: dict[str, Any], context: DomainContext) -> dict[str, Any]:
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            svc = OfferService(db)
            cancelled = await svc.cancel_draft(offer_id, company_id, user_id, reason)
            await db.commit()

        if not cancelled:
            return {"success": False, "error": "Proposta nao encontrada"}

        return {"success": True, "offer_id": str(offer_id), "status": "cancelled", "message": "Proposta cancelada"}
    except Exception as exc:
        logger.error("[OFFER-TOOL] cancel_offer failed: %s", exc, exc_info=True)
