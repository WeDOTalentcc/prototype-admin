...
def publish_node(state: JobCreationState) -> JobCreationState:
    ...
    if not os.environ.get("RAILS_API_URL"):
        raise RuntimeError(
            "publish_node requer RAILS_API_URL (caminho legacy de grafo). "
    ...
    try:
        ...
        if not job_id:
            ...
            job_data = {
                ...
                "variable_compensation": state.get("variable_compensation") or [],
        ...
        if job_id:
            ...
            if job_id and _mgr_email:
                try:
                    ...
                    _w1j_result = _W1J_CD().send_email(
                        to_email=_mgr_email,
                        subject=f"[WeDOTalent] Vaga criada: {_w1j_title} | Plano executivo",
                        body_html=_w1j_html,
                        from_name="LIA Recruitment",
                    )
                    logger.info(
                        "[publish] W1-J manager briefing sent to=%s ok=%s",
                        _mgr_email,
                        _w1j_result.get("success"),
                    )
