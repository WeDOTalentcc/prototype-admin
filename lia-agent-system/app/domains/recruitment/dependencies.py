...
def get_sub_status_repo(db: AsyncSession = Depends(get_db)) -> SubStatusRepository:
    return SubStatusRepository(db)


def get_ats_mapping_repo(db: AsyncSession = Depends(get_db)) -> ATSMappingRepository:
    return ATSMappingRepository(db)


def get_screening_question_repo(db: AsyncSession = Depends(get_db)) -> ScreeningQuestionRepository:
