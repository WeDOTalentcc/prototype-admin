"""
AlertRuleTemplateRepository - ADR-001 canonical.
...
class AlertRuleTemplateRepository:
    ...
    async def list_for_company(
        ...
        stmt = select(AlertRuleTemplate)

        scope_filter = AlertRuleTemplate.company_id == cid
        if include_master:
            scope_filter = or_(
                AlertRuleTemplate.is_master_template.is_(True),
                AlertRuleTemplate.company_id == cid,
            )

        stmt = stmt.where(scope_filter)
        if not include_deleted:
            stmt = stmt.where(AlertRuleTemplate.deleted_at.is_(None))
        stmt = stmt.order_by(AlertRuleTemplate.created_at.desc())
    ...
    ) -> AlertRuleTemplate | None:
        """Get template by id - only if master OR belongs to company (canonical scope)."""
        cid = self._require_company_id(company_id)
        stmt = select(AlertRuleTemplate).where(
            and_(
                AlertRuleTemplate.id == template_id,
                or_(
                    AlertRuleTemplate.is_master_template.is_(True),
                    AlertRuleTemplate.company_id == cid,
                ),
    ...
    async def customize_master(
        ...
        master_stmt = select(AlertRuleTemplate).where(
            and_(
                AlertRuleTemplate.id == master_id,
                AlertRuleTemplate.is_master_template.is_(True),
                AlertRuleTemplate.deleted_at.is_(None),
            )
        )
