...
Implements the BaseAgent interface using a ReAct loop with jobs-management-specific
tools, prompts and stage context. Manages the macro view of the job portfolio
including metrics, SLA compliance, and strategic actions. Follows the same
...
class JobsManagementReActAgent(LangGraphReActBase, EnhancedAgentMixin):
    """Autonomous agent for job portfolio management at the macro level.

    Implements the BaseAgent interface using a ReAct loop with
    jobs-management-specific tools, prompts and stage context. Each call to
    ...
    def _get_system_prompt(self, input: AgentInput) -> str:
        """System prompt do Jobs Management baseado no estágio atual."""
    ...
    async def _process_react_loop(self, input: AgentInput) -> AgentOutput:
        """Process a user message through the jobs management ReAct loop.
        ...
        logger.info(
            f"[JobsManagementReActAgent] Processing message for session={input.session_id} "
        ...
        try:
            ...
            # Sprint 3A — injeção de previsão de pipeline para o portfolio completo
            try:
                from app.services.pipeline_prediction_service import pipeline_prediction_service
                company_id_ctx = input.company_id or collected_fields.get("company_id", "")
                if company_id_ctx:
                    overview_data = await pipeline_prediction_service.get_company_overview(
                        company_id=company_id_ctx,
                    )
                    pred_block = get_pipeline_prediction_block(overview_data)
                    if pred_block:
            ...
            try:
                observer = ReActObserver(
                    ...
                    agent_class="JobsManagementReActAgent",
                    company_id=input.company_id,
                    user_id=input.user_id,
                )
                ...
                logger.warning(f"[JobsManagementReActAgent] Failed to create observer: {obs_err}")
                observer = None
            ...
            logger.info(
                f"[JobsManagementReActAgent] Completed in {duration_ms:.0f}ms "
            ...
            try:
                ...
                logger.warning(f"[JobsManagementReActAgent] Failed to finalize observer: {obs_err}")
            ...
            duration_ms = (time.time() - start_time) * 1000
            logger.error(
                f"[JobsManagementReActAgent] Error processing message: {exc}",
                exc_info=True,
            )
    ...
    async def _save_memory(
        ...
        try:
            ...
            if updates:
                ...
                logger.debug(
                    f"[JobsManagementReActAgent] Updated memory: {list(updates.keys())}"
                )
        except Exception as exc:
            logger.warning(
                f"[JobsManagementReActAgent] Failed to update memory: {exc}"
            )
    ...
    async def _extract_field_updates(
        ...
        for tc in state.tool_calls_made:
            ...
            if tool_name == "list_jobs":
                updates["portfolio_reviewed"] = True

            elif tool_name == "get_portfolio_metrics":
                updates["portfolio_reviewed"] = True
                data = result.get("data", {})
                if data.get("metrics_loaded"):
                    updates["metrics_summary"] = True

            elif tool_name == "compare_jobs":
                updates["analysis_completed"] = True
                updates["comparison_done"] = True

            elif tool_name == "check_sla":
                updates["analysis_completed"] = True
                updates["sla_checked"] = True

            elif tool_name == "analyze_bottlenecks":
                updates["analysis_completed"] = True
                updates["bottlenecks_identified"] = True

            elif tool_name in ("pause_job", "close_job", "reopen_job", "update_priority"):
    ...
    async def _check_navigation(
        ...
        if missing_required:
            logger.debug(
                f"[JobsManagementReActAgent] Cannot navigate: missing required fields "
        ...
        if not user_confirmed:
            logger.debug(
                f"[JobsManagementReActAgent] Required fields met for {current_stage} "
    ...
    def _build_error_output(self, exc: Exception, duration_ms: float) -> AgentOutput:
        """Build an error AgentOutput for exception cases.
