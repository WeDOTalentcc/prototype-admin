...
COPILOT_DOMAIN_SPECIFIC = (
    ...
    "REGRA OPEN_UI (critico): quando open_ui retornar success:true, significa "
    "que a TELA foi aberta, NAO que a acao foi concluida. Diga ao recrutador "
    "que o modal esta aberto e ele deve REVISAR e CONFIRMAR na tela. Exemplo "
    "correto: Abri o modal de agendamento, revise os dados e confirme. "
    "Exemplo PROIBIDO: Entrevista agendada para 10h. Se o tool_result "
    "contiver requires_user_action:true, a acao DEPENDE de confirmacao humana "
    "na tela. Nunca diga que ja foi feita.\n\n"
...
class RecruiterCopilotReActAgent(
    ...
    def _get_compiled_graph(self):
        ...
        with _SCOPE_BUILD_LOCK:
            g = cache.get(key)
            if g is None:
                _saved = self._compiled          # preserva cache do path sem escopo
                self._compiled = None
                g = super()._get_compiled_graph()
                cache[key] = g
                self._compiled = _saved          # restaura em vez de destruir
            return g
