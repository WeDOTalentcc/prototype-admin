"""
Talent ReAct Agent - Autonomous agent for talent funnel management.

Implements the BaseAgent interface using a ReAct loop with talent-specific
tools, prompts and stage context. Helps recruiters analyze, compare and
manage candidates in the talent funnel. Follows the same pattern as
PipelineReActAgent.
"""
import logging
import time
...
class TalentReActAgent(LangGraphReActBase, EnhancedAgentMixin):
    """Autonomous agent for the talent funnel domain.

    Implements the BaseAgent interface using a ReAct loop with
    talent-specific tools, prompts and stage context. Each call to
    ...
    def __init__(self) -> None:
        ...
        logger.info("[TalentReActAgent] Initialized")
    ...
    def _get_system_prompt(self, input: AgentInput) -> str:
        """System prompt do Talent baseado no estágio atual."""
    ...
    async def _process_react_loop(self, input: AgentInput) -> AgentOutput:
        """Process a user message through the talent funnel ReAct loop.
        ...
        logger.info(
            f"[TalentReActAgent] Processing message for session={input.session_id} "
        ...
        try:
            ...
            tools = get_talent_tools(current_stage) + self._get_all_enhanced_tools()

            system_prompt = get_talent_system_prompt(
                stage=current_stage,
                context={
            ...
            try:
                ...
                logger.warning(f"[TalentReActAgent] Failed to create observer: {obs_err}")
                observer = None
            ...
            logger.info(
                f"[TalentReActAgent] Completed in {duration_ms:.0f}ms "
            ...
            try:
                ...
                logger.warning(f"[TalentReActAgent] Failed to finalize observer: {obs_err}")
    ...
    def _build_error_output(self, exc: Exception, duration_ms: float) -> AgentOutput:
        ...
        Returns:
            AgentOutput with error information and friendly message.
        """
        return AgentOutput(
            message=(
    ...
    async def _save_memory(
        ...
        Args:
            ...
            session_id: Session identifier.
            current_stage: Current talent funnel stage.
        """
        try:
            ...
            if updates:
                ...
                logger.debug(
                    f"[TalentReActAgent] Updated memory: {list(updates.keys())}"
                )
        except Exception as exc:
            logger.warning(
                f"[TalentReActAgent] Failed to update memory: {exc}"
            )
    ...
    async def _extract_field_updates(
        ...
        for tc in state.tool_calls_made:
            ...
            if tool_name == "search_candidates":
                updates["search_criteria_defined"] = True
                candidates_found = result.get("data", {}).get("candidates_found")
                if candidates_found is not None:
                    updates["candidates_found"] = candidates_found

            elif tool_name == "list_candidates":
                updates["candidates_listed"] = True

            elif tool_name == "compare_candidates":
                updates["analysis_completed"] = True
                updates["comparison_done"] = True

            elif tool_name == "rank_candidates":
                updates["analysis_completed"] = True
                updates["ranking_generated"] = True

            elif tool_name == "analyze_skills":
                updates["skills_analyzed"] = True
                match_pct = result.get("data", {}).get("match_percentage")
                if match_pct is not None:
                    updates["match_percentage"] = match_pct

            elif tool_name == "recommend_actions":
                updates["recommendations_provided"] = True

            elif tool_name == "create_shortlist":
                updates["shortlist_created"] = True

            elif tool_name == "export_report":
                updates["report_exported"] = True
    ...
    async def _check_navigation(
        ...
        if missing_required:
            logger.debug(
                f"[TalentReActAgent] Cannot navigate: missing required fields "
        ...
        if not user_confirmed:
            logger.debug(
                f"[TalentReActAgent] Required fields met for {current_stage} "
