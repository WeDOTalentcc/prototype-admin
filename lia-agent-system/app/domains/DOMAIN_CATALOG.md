...
| Type | Criteria | Count |
|------|----------|-------|
| **Agentic** | Has `domain.py` with `@register_domain`, routable by orchestrator, implements `DomainPrompt` ABC | 16 |
| **Service** | Has `services/` with business logic but no `domain.py` — provides data access + domain services | 9 |
...
|--------|-------|-------------|
| `agent_studio` | 4 | Custom agent creation and management |
| `analytics` | 68 | Recruitment analytics, reports, dashboards |
| `ats_integration` | 25 | ATS system integration and sync |
| `automation` | 37 | Tasks, reminders, notes, workflow automation |
| `communication` | 75 | Email, WhatsApp, Teams messaging |
| `cv_screening` | 80 | CV analysis, WSI evaluation, candidate scoring |
| `digital_twin` | 3 | Digital twin creation and evaluation |
| `hiring_policy` | 14 | Hiring policy advisory with FairnessGuard |
| `interview_scheduling` | 25 | Interview scheduling and calendar management |
| `job_creation` | 13 | Wizard-driven job creation (WSI Phase B) |
| `job_management` | 69 | Job lifecycle management (CRUD, pipeline config) |
| `pipeline` (pipeline_transition) | 21 | Pipeline visualization and candidate movement |
| `recruiter_assistant` | 38 | General recruiter assistant (fallback domain) |
| `recruitment_campaign` | 3 | Multi-stage recruitment campaigns |
| `sourcing` | 49 | Candidate sourcing across channels |
| `talent_pool` | 3 | Talent pool management |
