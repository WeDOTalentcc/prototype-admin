...
class PlannedTaskService:
        ...
        if not include_completed:
            query = query.where(
                PlannedTask.status.notin_([
                    PlannedTaskStatus.COMPLETED,
                    PlannedTaskStatus.CANCELLED
                ])
            )
        
        query = query.order_by(PlannedTask.execution_level, PlannedTask.priority_score.desc())
    ...
    async def get_subtasks(
        ...
        result = await db.execute(
            ...
            .order_by(PlannedTask.execution_level, PlannedTask.priority_score.desc())
        )
        return list(result.scalars().all())
    ...
    async def build_task_dag(
        ...
        Returns:
            Dictionary with:
            - has_cycle: bool
            - cycle_path: list of task IDs if cycle exists
        ...
        if has_cycle:
            return {
                ...
                "error": "Ciclo detectado no grafo de dependências"
            }
    ...
    def _calculate_effort_efficiency(
        ...
        Quick wins (low effort) get higher scores.
        """
        ...
        if all_tasks is None:
            result = await db.execute(
                select(PlannedTask).where(
                    PlannedTask.goal_id == task.goal_id if task.goal_id 
                    else PlannedTask.parent_task_id == task.parent_task_id
                )
            )
    ...
    async def get_next_tasks(
        ...
        Returns tasks that:
        1. Are in READY or PENDING status
        ...
        completed_ids = set(row[0] for row in completed_result.fetchall())
    ...
    async def check_dependencies(
        ...
        Returns:
            ...
            - blocking_count: number of blocking dependencies
        """
        task = await self.get_task(db, task_id)
        if not task:
        ...
        result = await db.execute(
            select(PlannedTask).where(PlannedTask.id.in_(task.dependencies))
        )
