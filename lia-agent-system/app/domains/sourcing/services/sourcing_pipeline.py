            ...
            if total_added > 0:
                try:
                    ...
                    await event_dispatcher.on_candidates_sourced(
                        ...
                        candidates_found=len(local_candidates) + global_candidates_found,
                        candidates_added=total_added,
                        source="global" if expanded_to_global else "local",
                        expanded_to_global=expanded_to_global,
                        job_title=job.title
                    )
                except Exception as e:
                    logger.warning(f"Event dispatch failed for candidates sourced: {e}")
