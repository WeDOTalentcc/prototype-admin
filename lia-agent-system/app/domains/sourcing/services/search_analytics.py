...
class SearchAnalyticsService:
    ...
    async def generate_proactive_narrative_llm(
        self,
        analytics: dict[str, Any],
        search_criteria: dict[str, Any] | None = None
    ) -> str:
        """
        Generate a proactive narrative using Claude for richer, contextual insights.
        ...
        try:
            ...
            alerts = [a["message"] for a in analytics.get("alerts", [])]
            exp_range = analytics.get("experience_range", {})

            context = (
                f"Total: {summary.get('total_candidates', 0)} candidatos | "
                f"Score médio LIA: {summary.get('average_lia_score', 0)}% | "
                f"Exp. média: {exp_range.get('average', 0)} anos | "
                f"Top skills: {', '.join(top_skills)} | "
                f"Seniority: {distributions.get('seniority', {})} | "
                f"Localização: {distributions.get('location', {})} | "
                f"Alertas: {'; '.join(alerts) if alerts else 'nenhum'}"
            )
            if search_criteria:
    ...
    def generate_proactive_narrative(self, analytics: dict[str, Any]) -> str:
        """
        Generate a proactive narrative description of the search results (rule-based).
        For richer narratives, use generate_proactive_narrative_llm().
        """
        summary = analytics.get("summary", {})
