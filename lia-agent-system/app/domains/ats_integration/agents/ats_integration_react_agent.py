"""
ATS Integration ReAct Agent — Bidirectional ATS sync via ReAct loop.

Replaces the deprecated integrador_ats_agent.py (Sprint 6+).
...
class ATSIntegrationReActAgent(LangGraphReActBase, EnhancedAgentMixin):
    """ReAct agent for bidirectional ATS synchronization.

    Each call to ``process`` runs a full Reason-Act-Observe cycle using the
    ATS-integration-specific tools (sync_candidate_to_ats, fetch_candidate_from_ats,
    validate_ats_fields, bulk_sync_candidates, get_sync_status).
    ...
    def _get_system_prompt(self, input: AgentInput) -> str:
        """System prompt do ATS Integration."""
    ...
    async def _process_react_loop(self, input: AgentInput) -> AgentOutput:
        """Process an ATS integration request through the ReAct loop."""
        start_time = time.time()
        session_id = input.session_id
        company_id = input.company_id
        stage = input.context.get("current_stage", "provider-detection")

        logger.info(
            f"[ATSIntegrationReActAgent] process session={session_id} "
        ...
        try:
            ...
            system_prompt = get_ats_integration_system_prompt()
            ...
            try:
                observer = ReActObserver(
                    ...
                    domain="ats_integration",
                    agent_class="ATSIntegrationReActAgent",
                    company_id=company_id,
                    user_id=input.user_id,
                )
            except Exception as obs_err:
                logger.warning(
                    f"[ATSIntegrationReActAgent] Failed to create observer: {obs_err}"
                )
            ...
            logger.info(
                f"[ATSIntegrationReActAgent] Done session={session_id} "
            ...
            return AgentOutput(
                message=final_state.final_response or "Sincronização com ATS processada.",
                actions=actions,
            ...
            elapsed_ms = int((time.time() - start_time) * 1000)
            logger.error(
                f"[ATSIntegrationReActAgent] ReActLoop failed: {exc}", exc_info=True
            )
            return AgentOutput(
                message="Erro ao processar sincronização com ATS. Tente novamente.",
                confidence=0.0,
                error=str(exc),
