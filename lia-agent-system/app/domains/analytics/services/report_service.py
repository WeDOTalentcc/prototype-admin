...
async def generate_kpi_report(**params):
    """Wrapper KPIs semanais — usa demo data generator existente."""
    return {"kpis": _demo_gen.weekly_kpis(), "period": "weekly"}


async def analyze_funnel(**params):
    """Wrapper análise de funil — delega para demo data generator."""
    return _demo_gen.funnel_data()


async def job_health_check(**params):
    """Wrapper saúde da vaga — atalho para JobInsightsService.get_success_metrics."""
    raise NotImplementedError(
        "job_health_check via report_service não está implementado. "
        "Use analytics.get_job_health (job_insights_service) como alternativa."
    )


async def detect_anomalies(**params):
    """Detecção de anomalias — funcionalidade ainda não implementada."""
    raise NotImplementedError(
        "detect_anomalies ainda não está implementado. Backlog: integrar regras "
        "estatísticas (z-score sobre métricas históricas)."
    )


async def get_dashboard_data(**params):
    """Wrapper dashboard — combina KPIs estratégicos + breakdown por departamento."""
    return {
        "strategic_kpis": _demo_gen.strategic_kpis(),
        "executive_summary": _demo_gen.executive_summary(),
        "department_breakdown": _demo_gen.department_breakdown(),
        "predictions": _demo_gen.predictions(),
    }
