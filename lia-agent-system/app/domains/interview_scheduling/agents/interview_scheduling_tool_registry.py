...
Quando desativado, check_interviewer_availability e get_interview_status retornam
dict de erro honesto (calendar_not_configured) em vez de dados fabricados.
schedule_interview e reschedule_interview ainda persistem no DB, mas indicam
calendar_synced=False. Para ativar integração real, defina:
  CALENDAR_INTEGRATION_ENABLED=true (+ credenciais do provider no .env)
...
def get_interview_scheduling_tools() -> list[ToolDefinition]:
    ...
    Dois tools:
    - schedule_interview: agendamento + persistência no DB (real). Sincronização
      com calendário requer CALENDAR_INTEGRATION_ENABLED=true.
    - check_interviewer_availability: requer CALENDAR_INTEGRATION_ENABLED=true.
      Retorna calendar_not_configured quando desativado (sem dados fabricados).
    """
    return [
        ToolDefinition(
            ...
            description=(
                ...
                "Nota: sincronização com calendário externo (Google Calendar / Outlook) requer "
                "CALENDAR_INTEGRATION_ENABLED=true. Sem a flag, calendar_synced=False no retorno. "
        ...
        ToolDefinition(
            ...
            description=(
                ...
                "Requer CALENDAR_INTEGRATION_ENABLED=true. "
                "Retorna error='calendar_not_configured' quando a integração não está ativa "
