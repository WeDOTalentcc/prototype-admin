...
class MainOrchestrator:
    ...
    async def _try_wizard_canonical(
        ...
        # ── Phase 1.4a: Job creation disambiguation (Task #1204) ──────────
        # Flag-gated (JOB_CREATION_DISAMBIG_ENABLED, default OFF). Quando ligada,
        # a desambiguação ASSUME o roteamento dos turnos frescos de criação de
        # vaga (a continuação via is_session_active acima permanece intacta):
        #   • clarify  → LIA pergunta com opções estruturadas (pending store)
        #   • quick    → cria a vaga direto, sem descartar os detalhes da msg
        #   • guided   → delega ao wizard canônico (caminho abaixo)
        #   • search   → defere (return None) p/ Plan&Execute/agentic — o wizard
        #     NUNCA executa o composto "criar+buscar" (precedência explícita).
        # Substitui o bootstrap por substring legado enquanto a flag está ON.
        if not is_wizard_turn and self._is_job_creation_disambig_enabled():
            _kind, _payload = await self._handle_job_creation_disambiguation(
                ctx, conv_id, conv, db, message_text, session_id, company_id,
            )
            if _kind == "response":
                if (
                    conv and not ctx.skip_memory_persist
                    and _payload is not None and getattr(_payload, "content", None)
                ):
                    try:
                        await self._persist_response(
                            ctx, conv_id, conv, {"response": _payload.content}, db,
                        )
                    except Exception as _exc:
                        logger.warning(
                            "[MainOrchestrator] disambig persist failed: %s", _exc,
                        )
                return _payload
            if _kind == "defer":
                return None
            if _kind == "wizard":
                message_text = _payload or message_text
                is_wizard_turn = True

        # Bootstrap legado por substring — só quando a desambiguação está OFF
        # (com a flag ON, _handle_job_creation_disambiguation é a fonte única).
        if not is_wizard_turn and not self._is_job_creation_disambig_enabled():
    ...
    @staticmethod
    def _is_job_creation_disambig_enabled() -> bool:
        """Flag JOB_CREATION_DISAMBIG_ENABLED (env override > settings, default OFF)."""
        import os

        env = os.getenv("JOB_CREATION_DISAMBIG_ENABLED")
        if env is not None:
            return env.strip().lower() in ("1", "true", "yes", "on")
        try:
            from lia_config.config import settings

            return bool(getattr(settings, "JOB_CREATION_DISAMBIG_ENABLED", False))
        except Exception:
            return False

    @staticmethod
    def _build_disambig_chat_response(
        conv_id: str,
        content: str,
        *,
        intent: str,
        ...
        return ChatResponse(
            success=True,
            content=content,
            agent_used="job_creation_disambiguator",
            confidence=0.9,
            intent_detected=intent,
            conversation_id=conv_id,
    ...
    async def _handle_job_creation_disambiguation(
        self,
        ctx: UniversalContext,
        conv_id: str,
        conv: Any,
        ...
        """Decide o caminho de criação de vaga p/ um turno fresco (Task #1204).

        Retorna ``(kind, payload)`` onde ``kind`` é:
          • ``"response"`` — ``payload`` é um ``ChatResponse`` terminal (clarify,
            quick-create result, re-ask ou cancel ack).
          • ``"wizard"``   — ``payload`` é a mensagem efetiva p/ delegar ao wizard.
          • ``"defer"``    — ``payload`` é ``None``; caller faz ``return None``
            (defere o composto criar+buscar p/ Plan&Execute/agentic).
          • ``"skip"``     — não é criação de vaga; segue o fluxo normal.
        """
        ...
        from app.orchestrator.routing.job_creation_pending import (
            PendingJobCreation,
            pending_job_creation_store,
        )

        pending = pending_job_creation_store.get(session_id)

        # ── Turno de follow-up: recrutador respondendo a clarificação ──
        if pending is not None:
            choice = classify_path_choice(message_text)
            if choice == "cancel":
                pending_job_creation_store.remove(session_id)
                return (
                    "response",
                    self._build_disambig_chat_response(
                        conv_id,
                        "Sem problema — deixei a criação de vaga de lado por "
                        "enquanto. É só me chamar quando quiser retomar.",
                        intent="job_creation_disambiguation_cancel",
                    ),
                )
            if choice == JobCreationPath.CREATE_AND_SEARCH.value:
                pending_job_creation_store.remove(session_id)
                return ("defer", None)
            if choice == JobCreationPath.QUICK_CREATE.value:
                pending_job_creation_store.remove(session_id)
                return (
                    "response",
                    await self._execute_quick_create(
                        conv_id, pending.original_message, company_id,
                    ),
                )
            if choice == JobCreationPath.GUIDED_WIZARD.value:
                pending_job_creation_store.remove(session_id)
                return ("wizard", pending.original_message)

            # Resposta ambígua — re-pergunta uma vez; na 2ª, solta o pending e
            # reavalia a mensagem atual como turno fresco.
            if not pending.reasked:
                pending.reasked = True
                pending_job_creation_store.save(session_id, pending)
                return (
                    "response",
                    self._build_disambig_chat_response(
                        conv_id,
                        "Só pra eu garantir: você prefere que eu **crie a vaga "
                        "rápido** com o que você já me passou, ou que a gente "
                        "**monte passo a passo**? (ou me avise se quer "
                        "**cancelar**)",
                        intent="job_creation_disambiguation_reask",
                    ),
                )
            pending_job_creation_store.remove(session_id)
            # cai para a decisão fresca abaixo, usando a mensagem atual.

        # ── Turno fresco ──
        decision = decide_job_creation(
            message_text, plan_service_enabled=_is_plan_service_enabled(),
        )

        logger.info(
            "[MainOrchestrator] JobCreationDisambig: action=%s path=%s conf=%.3f "
            "gap=%.3f scores=%s session=%s",
            decision.action,
            decision.path.value if decision.path else "-",
            decision.confidence, decision.gap, decision.scores, session_id,
        )
        ...
        if decision.action == "clarify":
            pending_job_creation_store.save(
                session_id,
                PendingJobCreation(
                    conversation_id=session_id,
                    original_message=message_text,
                    option_paths=[p.value for p in decision.option_paths],
                ),
            )
            _lines = [decision.clarification_question or "", ""]
            for _i, _opt in enumerate(decision.clarification_options, 1):
                _lines.append(f"{_i}. {_opt}")
            return (
                "response",
                self._build_disambig_chat_response(
                    conv_id,
                    "\n".join(_lines).strip(),
                    intent="job_creation_disambiguation",
                    structured={
                        "job_creation_disambiguation": {
                            "options": decision.clarification_options,
                            "paths": [p.value for p in decision.option_paths],
        ...
        if decision.path == JobCreationPath.CREATE_AND_SEARCH:
            return ("defer", None)
        if decision.path == JobCreationPath.QUICK_CREATE:
            return (
                "response",
                await self._execute_quick_create(conv_id, message_text, company_id),
            )
        # GUIDED_WIZARD (inclui o fallback de quick sem job_title)
        return ("wizard", message_text)

    async def _execute_quick_create(
        self,
        conv_id: str,
        source_message: str,
        company_id: str | None,
    ) -> ChatResponse:
        """Criação rápida — cria a vaga direto a partir da mensagem do recrutador.

        Os detalhes da mensagem original NÃO são descartados: o título e o modelo
        de trabalho extraídos viram campos da vaga e a mensagem inteira vira a
        ``description`` (rascunho), de forma que nada se perde (Task #1204).
        """
        from app.orchestrator.routing.job_creation_disambiguator import (
            extract_quick_create_params,
        )

        params = extract_quick_create_params(source_message)
        title = params.get("job_title")
        if not title:
            # Salvaguarda — decide_job_creation já cai p/ guided sem job_title.
            return self._build_disambig_chat_response(
                conv_id,
                "Não consegui identificar o cargo da vaga. Qual posição você "
                "quer abrir?",
                intent="job_creation_quick_missing_title",
            )

        tool_params: dict[str, Any] = {"title": title, "description": source_message}
        if params.get("work_model"):
            tool_params["work_model"] = params["work_model"]

        try:
            from app.domains.job_management.tools import execute_job_management_tool
            ...
            logger.warning(
                "[MainOrchestrator] quick-create failed (non-blocking): %s",
                _exc, exc_info=True,
            )
            return self._build_disambig_chat_response(
                conv_id,
                "Tive um problema ao criar a vaga agora. Quer tentar de novo ou "
                "prefere montar passo a passo?",
                intent="job_creation_quick_error",
            )

        if not result or not result.get("success"):
            _msg = (result or {}).get("message") or "não foi possível criar a vaga"
            return self._build_disambig_chat_response(
                conv_id,
                f"Não consegui criar a vaga: {_msg}. Quer tentar de outro jeito?",
                intent="job_creation_quick_error",
            )

        _extras: list[str] = []
        if params.get("seniority"):
            _extras.append(str(params["seniority"]))
        if params.get("work_model"):
            _extras.append(str(params["work_model"]))
        if params.get("salary"):
            _extras.append(str(params["salary"]))
        _detail = f" ({', '.join(_extras)})" if _extras else ""

        _text = (
            f"✅ Criei a vaga **{result.get('title', title)}**{_detail} como "
            "rascunho. Quer que eu detalhe a descrição, configure o pipeline ou "
            "já comece a buscar candidatos?"
        )
        return self._build_disambig_chat_response(
            conv_id,
            _text,
            intent="job_creation_quick_created",
            structured={
                "job_vacancy": {
                    "id": result.get("id"),
