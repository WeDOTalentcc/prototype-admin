...
"""
    ...
    # ────────────────────────────────────────────────────────────────────
    # E10 — Carta-proposta (offer letter)
    # Versionado: bumpe OFFER_LETTER_VERSION quando o conteúdo mudar.
    # ────────────────────────────────────────────────────────────────────
    OFFER_LETTER_VERSION = "v1.0"

    @staticmethod
    def offer_letter(data: dict[str, Any]) -> dict[str, str]:
        """
        Build the offer-letter HTML + Markdown bodies for E10.
        ...
        Args:
            data: dict with keys:
                ...
                - benefits (list[str])
                - start_date (str | None — already formatted)
                - response_deadline (str | None — already formatted)
                - custom_clauses (list[str])
                - llm_body_markdown (str | None) — corpo opcional gerado por LLM
                  para a saudação/contexto, inserido entre o cabeçalho e os
                  blocos estruturados.

        Returns:
            {"html": "<...>", "markdown": "...", "version": "v1.0"}
        """
        candidate_name = data.get("candidate_name", "Candidato(a)")
        job_title = data.get("job_title", "")
        company_name = data.get("company_name", "Sua Empresa")
        manager_name = data.get("manager_name") or "Equipe de Recrutamento"
        currency = data.get("currency", "BRL")
        salary = data.get("salary")
        bonus_pct = data.get("bonus_pct")
        bonus_target = data.get("bonus_target")
        benefits = data.get("benefits") or []
        start_date = data.get("start_date") or "a combinar"
        response_deadline = data.get("response_deadline") or "5 dias úteis"
        custom_clauses = data.get("custom_clauses") or []
        llm_body = (data.get("llm_body_markdown") or "").strip()

        salary_line = (
            f"Salário base: {currency} {_format_number(salary, 2)} mensais"
            if salary is not None
            else "Salário base: a definir"
        )
        bonus_line = ""
        if bonus_pct is not None or bonus_target is not None:
            parts = []
            if bonus_target is not None:
                parts.append(f"alvo de {currency} {_format_number(bonus_target, 2)}")
            if bonus_pct is not None:
                parts.append(f"{_format_number(bonus_pct, 1)}% do salário")
            bonus_line = "Bônus variável: " + " · ".join(parts)

        benefits_md = "\n".join(f"- {b}" for b in benefits) if benefits else "- A combinar"
        clauses_md = "\n".join(f"- {c}" for c in custom_clauses) if custom_clauses else ""

        markdown = (
            f"# Carta-Proposta — {job_title}\n\n"
        ...
        if llm_body:
            ...
            markdown += (
                f"É com grande satisfação que a {company_name} formaliza esta proposta "
                f"para a posição de **{job_title}**, com base em sua trajetória ao longo "
                "do nosso processo seletivo.\n\n"
            )
        markdown += (
            "## Termos da Proposta\n\n"
            f"- {salary_line}\n"
            + (f"- {bonus_line}\n" if bonus_line else "")
            + f"- Data de início prevista: {start_date}\n"
            + f"- Prazo de resposta: {response_deadline}\n\n"
            "## Benefícios\n\n"
            f"{benefits_md}\n\n"
        )
        if clauses_md:
            markdown += "## Cláusulas adicionais\n\n" + clauses_md + "\n\n"
        markdown += (
            "## Próximos passos\n\n"
            "Caso aceite esta proposta, prosseguiremos com a coleta de documentos e "
            "o pré-onboarding. Caso queira negociar algum termo, basta responder a "
            "esta carta indicando os pontos de ajuste.\n\n"
            f"Atenciosamente,\n\n{manager_name}\n{company_name}\n"
        )
        ...
        benefits_html = (
            ...
            if benefits
            else "<li>A combinar</li>"
        )
        clauses_html = (
            "<h3 style='color:#1f2937;'>Cláusulas adicionais</h3><ul>"
            + "".join(f"<li>{c}</li>" for c in custom_clauses)
            + "</ul>"
            if custom_clauses
            else ""
        )
        llm_html = (
            f"<p>{llm_body.replace(chr(10), '<br>')}</p>" if llm_body else
            f"<p>É com grande satisfação que a {company_name} formaliza esta proposta "
            f"para a posição de <strong>{job_title}</strong>, com base em sua "
            "trajetória ao longo do nosso processo seletivo.</p>"
        )
        bonus_html = f"<li>{bonus_line}</li>" if bonus_line else ""
        html = f"""
<!DOCTYPE html>
<html><head><meta charset='utf-8'></head>
<body style="font-family:'Segoe UI',Tahoma,sans-serif;color:#1f2937;max-width:720px;margin:0 auto;padding:24px;background:#f9fafb;">
  <div style="background:white;border-radius:12px;padding:32px;box-shadow:0 2px 6px rgba(0,0,0,.06);">
    <h1 style="color:#111827;margin-top:0;">Carta-Proposta — {job_title}</h1>
    <p>Olá <strong>{candidate_name}</strong>,</p>
    {llm_html}
    <h3 style="color:#1f2937;">Termos da Proposta</h3>
    <ul>
      <li>{salary_line}</li>
      {bonus_html}
      <li>Data de início prevista: {start_date}</li>
      <li>Prazo de resposta: {response_deadline}</li>
    </ul>
    <h3 style="color:#1f2937;">Benefícios</h3>
    <ul>{benefits_html}</ul>
    {clauses_html}
    <h3 style="color:#1f2937;">Próximos passos</h3>
    <p>Caso aceite esta proposta, prosseguiremos com a coleta de documentos e o
       pré-onboarding. Caso queira negociar algum termo, basta responder a esta
       carta indicando os pontos de ajuste.</p>
    <p style="margin-top:32px;">Atenciosamente,<br><strong>{manager_name}</strong><br>{company_name}</p>
  </div>
  <p style="text-align:center;color:#9ca3af;font-size:11px;margin-top:16px;">
    Documento gerado pela LIA · template {ReportTemplates.OFFER_LETTER_VERSION}
  </p>
</body></html>
"""
        return {
            "html": html,
            "markdown": markdown,
            "version": ReportTemplates.OFFER_LETTER_VERSION,
        }
