...
class CampaignRepository:
    ...
    async def list_by_job_id_in_array(
        self,
        company_id: str,
        job_id: str,
        limit: int = 50,
    ) -> list[RecruitmentCampaign]:
        """Find campaigns where job_ids array contains the given job_id (GIN index)."""
        self._require_company_id(company_id)
        try:
            job_uuid = _uuid.UUID(job_id)
        except ValueError:
            return []
        # PostgreSQL: job_ids @> ARRAY[job_uuid::uuid]
        q = (
            select(RecruitmentCampaign)
            .where(
                RecruitmentCampaign.company_id == company_id,
                RecruitmentCampaign.job_ids.contains([job_uuid]),
            )
