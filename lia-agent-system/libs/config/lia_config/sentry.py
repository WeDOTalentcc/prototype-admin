...
def _normalize_path(path: str) -> str:
    """Replace UUIDs and numeric IDs in paths to improve Sentry grouping."""
    path = re.sub(r'[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}', '{uuid}', path)
    path = re.sub(r'/\d+', '/{id}', path)
    return path


def _add_fingerprint(event: dict, hint: dict) -> dict:
    """
    GAP-11-023: Custom Sentry fingerprinting for better issue grouping.

    Rules (evaluated in priority order):
    1. LIAError subclasses  -> ["lia-error", ClassName]
    2. HTTPException        -> ["http", status_code, normalized_path]
    3. SQLAlchemy errors    -> ["db-error", ExcClassName]
    4. LangGraph / LLM      -> ["llm-error", ExcClassName]
    5. Everything else      -> Sentry default (no fingerprint override)
    """
    ...
    try:
        ...
        _http_exc_cls = HTTPException
    except ImportError:
        pass

    if _lia_error_cls is not None and isinstance(exc, _lia_error_cls):
        event["fingerprint"] = ["lia-error", exc_name]

    elif _http_exc_cls is not None and isinstance(exc, _http_exc_cls):
        request_info = event.get("request", {})
        raw_url = request_info.get("url", "")
        try:
        ...
        event["fingerprint"] = ["http", str(exc.status_code), normalized]

    elif "sqlalchemy" in exc_module:
        event["fingerprint"] = ["db-error", exc_name]

    elif any(mod in exc_module for mod in ("langgraph", "langchain", "anthropic", "openai")):
        event["fingerprint"] = ["llm-error", exc_name]
...
def _before_send(event: dict, hint: dict) -> dict | None:
    ...
    # GAP-11-023: add custom fingerprint for better Sentry grouping
    event = _add_fingerprint(event, hint)
