...
class TestBigFiveProfile:
    ...
    def test_with_scores(self):
        ...
        assert m.openness == pytest.approx(0.8)
        assert m.conscientiousness == pytest.approx(0.7)

    def test_partial_scores(self):
        m = BigFiveProfile(openness=0.9)
        assert m.openness == pytest.approx(0.9)


class TestRegenerateQuestionsRequest:
    def test_basic(self):
        m = RegenerateQuestionsRequest(context="Senior Python developer role")
        assert m.context == "Senior Python developer role"

    def test_with_extra(self):
        m = RegenerateQuestionsRequest(
            context="Backend developer",
            block_id="block-001",
            num_questions=3,
        )
        assert m.context == "Backend developer"
...
class TestScreeningQuestionResponse:
    ...
    def test_with_questions(self):
        ...
        m = ScreeningQuestionResponse(questions=[q], total=1)
        assert m.total == 1
...
class TestWSIBlockSummary:
    def test_basic(self):
        q1 = UnifiedScreeningQuestion(
            id="q1", text="Q1", category="technical", block_id="b1"
        )
        m = WSIBlockSummary(
            block_id="b1",
        ...
        assert m.block_id == "b1"
        assert m.question_count == 1
        assert len(m.questions) == 1

    def test_multiple_questions(self):
        questions = [
            UnifiedScreeningQuestion(id=f"q{i}", text=f"Q{i}", category="behavioral", block_id="b2")
            for i in range(3)
        ]
        m = WSIBlockSummary(
            block_id="b2",
...
class TestWSIScreeningPipelineResponse:
    def test_empty(self):
        m = WSIScreeningPipelineResponse()
        assert m is not None
    ...
    def test_with_blocks(self):
        block = WSIBlockSummary(
            ...
            question_count=2,
            questions=[
                UnifiedScreeningQuestion(id="q1", text="Q1", category="technical", block_id="b1"),
                UnifiedScreeningQuestion(id="q2", text="Q2", category="technical", block_id="b1"),
            ],
        )
        m = WSIScreeningPipelineResponse(blocks=[block], total_questions=2)
