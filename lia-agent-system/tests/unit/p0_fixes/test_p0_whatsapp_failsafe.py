...
def test_whatsapp_service_configured_without_any_number():
    """is_configured=False quando nem TWILIO_WHATSAPP_NUMBER nem TWILIO_VOICE_NUMBER estão presentes."""
    env = {k: v for k, v in os.environ.items()
           if k not in ("TWILIO_WHATSAPP_NUMBER", "TWILIO_VOICE_NUMBER")}
    ...
    with patch.dict(os.environ, env, clear=True):
        ...
        assert not svc.is_configured, (
            "is_configured deveria ser False sem nenhum número WhatsApp/Voice configurado"
        )


def test_whatsapp_service_configured_with_voice_number_fallback():
    """is_configured=True quando TWILIO_VOICE_NUMBER é usado como fallback para WhatsApp."""
    env = {k: v for k, v in os.environ.items()
           if k not in ("TWILIO_WHATSAPP_NUMBER",)}
    env["TWILIO_ACCOUNT_SID"] = "AC76fd82fbff27b6d334ea540ec6a1840b"
    env["TWILIO_AUTH_TOKEN"] = "1b3319733bf330ec307ada46f617e52a"
    env["TWILIO_VOICE_NUMBER"] = "+551140403098"
    with patch.dict(os.environ, env, clear=True):
        import importlib
        import app.domains.communication.services.whatsapp_twilio_service as mod
        importlib.reload(mod)
        ...
        assert svc.is_configured, (
            "is_configured deveria ser True usando TWILIO_VOICE_NUMBER como fallback"
        )
        assert svc.whatsapp_number == "+551140403098"
