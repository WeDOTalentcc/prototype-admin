...
# ─────────────────────────────────────────────────────────────────────────────
# Integration tests — PolicyGateService com PolicyEngine real
# ─────────────────────────────────────────────────────────────────────────────


class TestPolicyGateServiceIntegration:
    """Integration tests usando PolicyEngine real (sem DB)."""

    @pytest.fixture
    def real_policy_engine(self):
        """PolicyEngine real, sem DB (db_service=None)."""
        from app.orchestrator.policy_engine import PolicyEngine

        return PolicyEngine(db_service=None)

    @pytest.mark.asyncio
    async def test_safe_intent_with_real_engine(self, real_policy_engine):
        """Fast-path funciona com PolicyEngine real (sem chamar DB)."""
        gate = PolicyGateService(policy_engine=real_policy_engine)
        result = await gate.validate(
            intent="general_chat",
            user_id="u1",
            context={"company_id": "tenant-test"},
        )
        assert result.allowed is True

    @pytest.mark.asyncio
    async def test_unknown_intent_returns_default_allowed(self, real_policy_engine):
        """PolicyEngine permite intents desconhecidos por default (validate_request fallthrough)."""
        gate = PolicyGateService(policy_engine=real_policy_engine)
        result = await gate.validate(
            intent="some_unknown_intent_xyz",
            user_id="u1",
            context={"company_id": "tenant-test"},
        )
        # Comportamento esperado do PolicyEngine real: allowed por default
        # (apenas search/screening/communication têm validações específicas)
        assert isinstance(result, PolicyResult)
        # allowed pode ser True ou False — captura o comportamento real
