...
class WizardNodeMessagesT1099(unittest.TestCase):
    ...
    # ──────────────────────────────────────────────────────── publish
    def test_publish_normal_populates_message(self):
        from app.domains.job_creation.graph import publish_node
        state = _base_state(
            jd_enriched={"titulo_padronizado": "Eng Backend",
                         "about_role": "x"},
            seniority_resolved="pleno",
            wsi_questions=[{"question": "q"}],
            publish_platforms=["website"],
            policy_confirmed_publish=True,
        )

        fake_resp = MagicMock(success=True, data={
            "data": {"id": "job-123", "uid": "uid-xyz"},
        })
        fake_link = MagicMock(success=True, data={
            "share_link": "https://share.example/j/123",
        })
        fake_api = MagicMock()
        fake_api.create_job.return_value = fake_resp
        fake_api.save_screening_config.return_value = MagicMock(success=True)
        fake_api.publish_job.return_value = MagicMock(success=True)
        fake_api.get_share_link.return_value = fake_link
        ...
        _assert_msg(self, result, where="publish[normal]",
                    must_contain=("publicada",))

    def test_publish_error_populates_message(self):
        from app.domains.job_creation.graph import publish_node
        state = _base_state(
            jd_enriched={"titulo_padronizado": "x", "about_role": "x"},
            seniority_resolved="pleno",
            policy_confirmed_publish=True,
        )
        fake_api = MagicMock()
        fake_api.create_job.side_effect = RuntimeError("rails 503")
        with patch(
            "app.domains.job_creation.graph._get_api_client",
            return_value=fake_api,
        ):
            result = publish_node(state)
        _assert_msg(self, result, where="publish[error]",
                    must_contain=("rails 503",))

    # ──────────────────────────────────────────────────────── calibration
    def test_calibration_populates_message(self):
        from app.domains.job_creation.graph import calibration_node
        state = _base_state(
            job_id="job-1",
            calibration_candidates=[
                {"id": "c1", "name": "A", "decision": "approved"},
                {"id": "c2", "name": "B", "decision": "approved"},
                {"id": "c3", "name": "C", "decision": "approved"},
            ],
            calibration_threshold=3,
        )
        result = calibration_node(state)
        _assert_msg(self, result, where="calibration[complete]",
                    must_contain=("3/3",))

    def test_calibration_partial_populates_message(self):
        from app.domains.job_creation.graph import calibration_node
        state = _base_state(
            job_id="job-1",
            calibration_candidates=[
        ...
        result = calibration_node(state)
        _assert_msg(self, result, where="calibration[partial]",
                    must_contain=("1/3",))

    # ──────────────────────────────────────────────────────── handoff
    def test_handoff_populates_message(self):
        from app.domains.job_creation.graph import handoff_node
        state = _base_state(
            job_id="job-1",
            share_link="https://share.example/j/1",
        )
        result = handoff_node(state)
        _assert_msg(self, result, where="handoff[normal]",
                    must_contain=("share.example",))
