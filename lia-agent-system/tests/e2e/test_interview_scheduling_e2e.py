...
class TestInterviewGraphTracing:
    ...
    async def test_run_node_logs_start_end(self):
        ...
        logging.getLogger("InterviewGraph").addHandler(handler)
        logging.getLogger("InterviewGraph").setLevel(logging.DEBUG)

        state = {"workflow_data": {}, "session_id": "trace-test"}

        async def mock_node(s):
            return s

        g._node_fns["interview_state_loader"] = mock_node
        await g._run_node("interview_state_loader", state)

        logging.getLogger("InterviewGraph").removeHandler(handler)

        assert any("node_start" in m for m in log_records)
    ...
    async def test_run_node_error_logs_node_error(self):
        ...
        g._node_fns["interview_state_loader"] = failing_node
        state = {"workflow_data": {}, "session_id": "trace-test-err"}
        result = await g._run_node("interview_state_loader", state)

        logging.getLogger("InterviewGraph").removeHandler(handler)

        assert any("node_error" in m for m in log_records)
        assert "interview_graph_error" in result["workflow_data"]
