"""
Tests for Sprint 1 — WSI canonical constants and FormacaoPreQualifierResult schema.
"""
import pytest
from app.domains.cv_screening.constants.wsi_constants import (
    WSI_DIMENSION_LABELS,
    WSI_DIMENSION_WEIGHTS_DEFAULT,
    WSI_BLOCK_NAMES,
    FORMACAO_PRE_QUALIFIER_LABEL,
)
from app.schemas.screening import FormacaoPreQualifierResult


class TestWSIDimensionLabels:
    def test_four_canonical_dimensions_exist(self):
        assert set(WSI_DIMENSION_LABELS.keys()) == {"technical", "behavioral", "gap_analysis", "contextual"}

    def test_dimension_labels_are_portuguese(self):
        assert WSI_DIMENSION_LABELS["technical"] == "Competências Técnicas"
        assert WSI_DIMENSION_LABELS["behavioral"] == "Competências Comportamentais"
        assert WSI_DIMENSION_LABELS["gap_analysis"] == "Experiência Profissional"
        assert WSI_DIMENSION_LABELS["contextual"] == "Fit Cultural e Alinhamento"

    def test_weights_sum_to_one(self):
        total = sum(WSI_DIMENSION_WEIGHTS_DEFAULT.values())
        assert abs(total - 1.0) < 1e-9, f"Weights sum to {total}, expected 1.0"

    def test_weights_match_guia_v3(self):
        assert WSI_DIMENSION_WEIGHTS_DEFAULT["technical"] == 0.50
        assert WSI_DIMENSION_WEIGHTS_DEFAULT["behavioral"] == 0.20
        assert WSI_DIMENSION_WEIGHTS_DEFAULT["gap_analysis"] == 0.15
        assert WSI_DIMENSION_WEIGHTS_DEFAULT["contextual"] == 0.15


class TestWSIBlockNames:
    def test_seven_blocks_defined(self):
        assert len(WSI_BLOCK_NAMES) == 7

    def test_block_3_renamed_elegibilidade(self):
        # Bloco 3 deve conter "Elegibilidade" ou "Formação"
        name = WSI_BLOCK_NAMES[3]
        assert "Elegibilidade" in name or "Formação" in name, (
            f"Block 3 name '{name}' should mention Elegibilidade or Formação"
        )

    def test_block_4_aligns_with_dimension(self):
        assert "Técnic" in WSI_BLOCK_NAMES[4], (
            f"Block 4 '{WSI_BLOCK_NAMES[4]}' should align with 'Competências Técnicas'"
        )

    def test_block_5_aligns_with_dimension(self):
        name = WSI_BLOCK_NAMES[5]
        assert "Comportamental" in name or "Fit" in name, (
            f"Block 5 '{name}' should align with behavioral dimension"
        )

    def test_no_old_labels_remain(self):
        old_labels = {"Análise de Gaps", "Contexto e Motivação", "Perguntas Finais"}
        all_names = set(WSI_BLOCK_NAMES.values())
        overlap = old_labels & all_names
        assert not overlap, f"Deprecated labels still present: {overlap}"


class TestFormacaoPreQualifierResult:
    def test_passes_true_basic(self):
        result = FormacaoPreQualifierResult(
            required_certifications=["OAB"],
            candidate_certifications=["OAB"],
            passes=True,
        )
        assert result.passes is True

    def test_passes_false_basic(self):
        result = FormacaoPreQualifierResult(
            required_certifications=["CRM"],
            candidate_certifications=[],
            passes=False,
        )
        assert result.passes is False

    def test_note_default_empty_string(self):
        result = FormacaoPreQualifierResult(passes=True)
        assert result.note == ""

    def test_note_contextual_accepted(self):
        result = FormacaoPreQualifierResult(
            passes=True,
            note="Bootcamp equivale a diploma para esta vaga conforme critério da empresa.",
        )
        assert "Bootcamp" in result.note

    def test_certifications_default_empty_lists(self):
        result = FormacaoPreQualifierResult(passes=True)
        assert result.required_certifications == []
        assert result.candidate_certifications == []
