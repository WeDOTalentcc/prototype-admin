...
def test_eleven_canonical_concepts():
    assert len(POLICY_INSTRUCTION_KEYS) == 7  # V2.2: 4 redundantes removidas (D&I/remoto/salário/janela)
...
def test_accepts_known_keys_and_strips():
    out = validate_policy_instructions({
        "screening_criteria": "  Mínimo 3 anos em gestão.  ",
...
def test_v2_removed_keys_now_rejected():
    """V2.2: D&I/remoto/salário/janela saíram de Políticas (vão para LIA fields / removidas)."""
    import pytest as _pytest
    for removed in ("diversity_inclusion_guidelines", "remote_work_policy",
                    "salary_negotiation_policy", "communication_window"):
        with _pytest.raises(PolicyBlockValidationError):
            validate_policy_instructions({removed: "x"})
...
def test_none_becomes_empty_string():
    out = validate_policy_instructions({"no_show_policy": None})
    assert out == {"no_show_policy": ""}
