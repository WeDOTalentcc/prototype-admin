...
| 15 | 🟡 | **Cache key de embedding sem `company_id`** (bug adjacente ao Gap E): `EmbeddingCacheService` não inclui `company_id` na chave Redis. Para tenants BYOK com modelo de embedding diferente, um vetor gerado com a chave/modelo de um tenant pode ser servido a outro (falso-positivo de cache → inconsistência BYOK; não é vazamento de texto de candidato, pois o embedding é do texto). Avaliar escopo separado. | §8.3 | `app/shared/services/embedding_cache_service.py` |
| 16 | 🟡 | **`talent_intelligence` tools usam `kwargs.get('company_id')`** em vez do padrão fail-closed canônico (`@tool_handler` + ContextVar). Um `company_id` ausente vira string vazia e o tool prossegue com tenant-check quebrado em vez de levantar. Revisar os 15 tools antes de promovê-los a primários no escopo do federado. | §14.2 | `app/domains/talent_intelligence/tools/` |
