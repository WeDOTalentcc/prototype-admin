...
## Production setup — checklist executável

> Esta seção é o passo-a-passo de plantão para deixar o app da LIA no Teams
> rodando em produção (Task #706). Sempre que algo quebrar em produção,
> comece por aqui.

### Valores reais — registro vivo (a ser preenchido pelo operador no Azure)

> Esta tabela é um **registro vivo**: quem executar o passo 1 do playbook
> ("Descoberta no Azure") deve substituir cada `<preencher>` pelo valor real
> obtido no `portal.azure.com` e fazer commit. **Os valores aqui são
> identificadores não-sensíveis** (Client IDs, Tenant IDs, domínio, App ID
> URI) — secrets ficam exclusivamente no cofre de produção, nunca aqui.

| Item | Valor |
|------|-------|
| Domínio público de produção | `https://ai.wedotalent.cc` |
| Bot resource group (Azure) | `lia-bot-rg` |
| Bot handle | `LIA-WeDOTalent` |
| Microsoft App ID do bot (`MICROSOFT_APP_ID`) | `<preencher>` |
| Tenant home do bot (`TEAMS_APP_TENANT_ID`) | `<preencher>` |
| App Registration de SSO/Graph | mesmo do bot? `<sim/não>` — se separado, anotar nome |
| Azure Client ID de SSO/Graph (`AZURE_CLIENT_ID`) | `<preencher>` |
| Azure Tenant ID de SSO/Graph (`AZURE_TENANT_ID`) | `<preencher>` |
| Application ID URI exposta | `api://ai.wedotalent.cc/<AZURE_CLIENT_ID>` |
| Scope exposto | `access_as_user` |
| `TEAMS_APP_ID` (UUID estável do app no Teams) | `<preencher uma vez e nunca trocar>` |
| Mapping `tenant_id Teams → company_id LIA` populado em prod | `<sim/não — listar tenants atendidos>` |

### Endpoints de produção (devem existir)

```
Messaging endpoint (Azure Bot):  https://ai.wedotalent.cc/api/v1/teams/messages
Redirect URI SSO (Azure App):    https://ai.wedotalent.cc/api/v1/teams/auth/callback
Redirect URI SSO page:           https://ai.wedotalent.cc/api/v1/teams/auth/sso-page
Manifesto dinâmico:              https://ai.wedotalent.cc/api/v1/teams/manifest
Pacote ZIP (upload no Teams):    https://ai.wedotalent.cc/api/v1/teams/manifest-zip
Health do bot:                   https://ai.wedotalent.cc/api/v1/teams/health
Ícones servidos:                 https://ai.wedotalent.cc/teams-icons/wedo-color.png
...
```bash
# Bot (Azure Bot + App Registration do bot)
MICROSOFT_APP_ID=<client id do bot>
MICROSOFT_APP_PASSWORD=<client secret do bot>     # rotacionar a cada 6-12 meses
TEAMS_APP_TENANT_ID=<tenant home do bot>          # NÃO confundir com AZURE_TENANT_ID

# SSO + Microsoft Graph
AZURE_CLIENT_ID=<client id do app de SSO/Graph>
AZURE_CLIENT_SECRET=<client secret do app de SSO/Graph>
AZURE_TENANT_ID=<tenant do app de SSO/Graph>

# Webhook de Adaptive Card
TEAMS_WEBHOOK_SECRET=<32+ bytes aleatórios — gerar com `openssl rand -hex 32`>

# Plataforma
WEDOTALENT_PLATFORM_URL=https://ai.wedotalent.cc

# UUID estável do app no Teams (se não for setado, o backend recusa em prod)
TEAMS_APP_ID=<gerar uma vez com uuidgen e fixar>
```

### Passo-a-passo de configuração

1. **Descoberta no Azure.** No `portal.azure.com`, navegar até o resource group `lia-bot-rg`, abrir `LIA-WeDOTalent`, ir em **Configuration** → anotar Microsoft App ID, messaging endpoint atual, App Registration vinculado. Em **App registrations**, identificar se o SSO usa o mesmo app ou outro separado, e anotar Tenant ID. Preencher a tabela acima.

2. **Messaging endpoint + redirect URIs.** No Azure Bot → **Configuration**, garantir que o messaging endpoint aponta para `https://ai.wedotalent.cc/api/v1/teams/messages`. No App Registration de SSO → **Authentication** → **Redirect URIs (Web)**, garantir que existem `https://ai.wedotalent.cc/api/v1/teams/auth/callback` e `https://ai.wedotalent.cc/api/v1/teams/auth/sso-page`.

3. **Permissions de Graph + admin consent.** No App Registration de SSO/Graph → **API permissions**, garantir as permissões: `User.Read`, `Calendars.ReadWrite`, `OnlineMeetings.ReadWrite`, `offline_access`, `openid`, `profile`, `email`. Clicar em **Grant admin consent for <tenant>**. Conferir que a coluna **Status** mostra "Granted for ..." para todas.

4. **Expose API + pre-authorized clients.** No App Registration → **Expose an API** → **Set Application ID URI** = `api://ai.wedotalent.cc/<AZURE_CLIENT_ID>`. Adicionar um scope `access_as_user`. Em **Authorized client applications**, pré-autorizar os Client IDs oficiais do Teams: `1fec8e78-bce4-4aaf-ab1b-5451cc387264` (Teams mobile/desktop), `5e3ce6c0-2b1f-4285-8d4b-75ee78787346` (Teams web). Sem isso o SSO silencioso da aba não funciona.

5. **Gerar/rotacionar secrets.** No App Registration do bot → **Certificates & secrets** → **New client secret** (válido 12 meses). No App Registration de SSO/Graph, idem. Gerar `TEAMS_WEBHOOK_SECRET` com `openssl rand -hex 32`. Guardar tudo no cofre de produção.

6. **Popular env vars em prod.** Adicionar todas as variáveis listadas acima no ambiente do backend de produção. Reiniciar o backend e conferir nos logs que não há mensagens de "credentials not configured".

7. **Validar manifesto.** Rodar o validador automatizado (próxima seção). Ele bate manifest, ícones, health e (opcional) consegue token do Bot Framework.

8. **Mapping tenant → company.** No banco de produção, inserir/confirmar a relação entre `tenant_id` do Teams e `company_id` da LIA, para a empresa da WeDO e para o tenant piloto. Sem isso a ponte de orquestração rejeita as mensagens.

9. **Distribuir no Teams Admin Center.** Baixar o pacote: `curl -L -o wedo.zip https://ai.wedotalent.cc/api/v1/teams/manifest-zip`. Em `admin.teams.microsoft.com` → **Teams apps** → **Manage apps** → **Upload new app**. Aprovar política de instalação para o grupo de recrutadores piloto.

10. **Smoke test ponta a ponta** (no Teams, conta real de recrutador):
    - (a) Instalar o app.
    - (b) `/ajuda` → resposta da LIA chega.
    - (c) `/buscar <critério>` → LIA responde.
    - (d) Abrir aba pessoal → entra logado por SSO sem digitar senha.
    - (e) Disparar Adaptive Card de aprovação, clicar num botão; nos logs do backend, confirmar `[TEAMS WEBHOOK] Received action=approve` e que a assinatura HMAC foi validada.
    - (f) Agendar entrevista de teste; confirmar reunião real no calendário Teams via Graph.

11. **Registro final.** Atualizar a tabela "Valores reais" no topo desta seção com o que foi descoberto. Sem secrets.

### Validador automatizado

Rodar o script abaixo bate todas as checagens automatizáveis em uma única
chamada. Use sempre que algo quebrar ou após qualquer mudança de configuração:

```bash
# Apenas checagens públicas (não precisa de credenciais locais)
python lia-agent-system/scripts/validate_teams_production.py \
    --base-url https://ai.wedotalent.cc

# Inclui acquisição de token Bot Framework com as credenciais locais
# (use um .env apontado para os valores de produção, ou exporte antes)
python lia-agent-system/scripts/validate_teams_production.py \
    --base-url https://ai.wedotalent.cc --check-token
```

Saída:
```
Validating Teams production setup against https://ai.wedotalent.cc

Environment variables (local process):
  [PASS] env: MICROSOFT_APP_ID — present
  ...
Backend endpoints:
  [PASS] teams/health — healthy + bot_configured
  [PASS] teams/manifest — structure ok
  [PASS] icon: wedo-color.png — HTTP 200 (xxxx bytes)
  [PASS] icon: wedo-outline.png — HTTP 200 (xxxx bytes)

Summary: N/N checks passed
```

Exit code 0 se tudo passar. Qualquer falha vira exit code 1 e aparece em
"Failures:" com o motivo concreto (env faltando, manifest sem
`webApplicationInfo`, ícone 404, token rejeitado pelo tenant, etc.).
