...
Context (Fase 2.5 Onda C2.1 — AUDIT 6 P0-1):
  ...
  DEFERRED FKs (NOT applied here — see final report):
  1. custom_agents.company_id -> companies.id : 42 of 50 rows are ephemeral test
     fixtures (company_id '00000000-0000-0000-0000-000000000001' + 'test-7c-*'
     from contract runs) with no matching companies row. Applying the FK would
     fail; deleting the rows requires authorization. Apply after the dev DB is
     cleaned.
  2. ai_consumption.studio_agent_id -> custom_agents.id : type mismatch. The
     column is varchar(64) (treated as str across ~15 call sites: API query
     params, AiConsumption.studio_agent_id == <str>, str(row...)). custom_agents.id
     is uuid. A FK needs comparable types; converting the column to uuid has a
     wide blast radius on the consumption read paths. Since this is a SET NULL
     historical reference (not integrity-critical), it is deferred pending a
     dedicated type-alignment migration + caller audit.
"""
