"""Sprint P.1 fix — deferred tables that 148 placeholder failed to apply.

Revision ID: 151_sprint_p1_deferred_tables_fix
Revises: 150_sprint_p3_d2_drop_safe
Create Date: 2026-05-21

ROOT CAUSE: Migration 148 was created as a slot-claim placeholder during the
P.1/P.2/P.3 parallel race. P.2 ran alembic upgrade head while 148 still
had def upgrade(): pass body, so alembic stamped 148 = applied without
running real DDL. P.2 explicitly documented this expectation in its docstring:
Tables claimed by Sprint P.1 ... intentionally NOT included even though P.1
migration 148 shipped as a no-op claim. Re-saving 148 with real DDL did not
re-trigger because alembic tracks revision, not file content.

This migration carries the actual DDL deferred from Sprint G migration 143
for the 3 tables P.1 was assigned:

1. whatsapp_conversations (13 cols) — pre-qualification + eligibility +
   reconsideration + existing-candidate detection fields.
2. profile_calculation_logs (9 cols) — recruiter profile recalculation audit.
3. ai_consumption (2 cols) — agent_category (NOT NULL server_default=core)
   + studio_agent_id (nullable), both indexed.

Tables verified empty (row_count=0 each) at migration time, so NOT NULL adds
are safe even without server_default. Server defaults applied where model
declares default= to preserve canonical semantics.
...
revision = "151_sprint_p1_deferred_tables_fix"
down_revision = "150_sprint_p3_d2_drop_safe"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    # Trigger + analysis counters (table empty, NOT NULL safe; server_default scaffold then dropped)
    ...
    # Drop server_default for trigger (canonical model has no default — server_default was only
    # safe-migration scaffold for non-empty tables). Empty table here so functionally a no-op.
