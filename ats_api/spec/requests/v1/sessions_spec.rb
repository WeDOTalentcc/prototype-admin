...
RSpec.describe "V1::Sessions API", type: :request do
  ...
  describe "POST /v1/sessions" do
    ...
    context "when password login is disabled" do
      ...
      let(:user_with_sso_only) { create(:user, account: account, password: "password123") }

      it "returns forbidden" do
        post "/v1/sessions", params: {
          email: user_with_sso_only.email,
          password: "password123"
        }.to_json, headers: headers

        expect(response).to have_http_status(:forbidden)
        json = JSON.parse(response.body)
        expect(json["error"]).to eq("Password login is not enabled for this account")
        expect(json["available_methods"]).to be_present
        expect(json["available_methods"]).to include("microsoft_entra_id")
      end
    end
  end
  ...
  describe "GET /v1/users/me" do
    context "with valid token" do
      ...
      it "returns current user" do
        get "/v1/users/me", headers: headers.merge("Authorization" => "Bearer #{token}")
    ...
    context "without token" do
      it "returns unauthorized" do
        get "/v1/users/me", headers: headers
