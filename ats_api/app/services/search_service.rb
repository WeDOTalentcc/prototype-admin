...
class SearchService
  def self.exist_search?(class_current)
    ...
    return false unless index_healthy?(class_current)

    true
  end

  def self.index_healthy?(class_current)
    return true unless class_current.respond_to?(:search_index)

    index = class_current.search_index
    return false unless index.exists?

    true
  rescue StandardError => e
    Rails.logger.warn "[SearchService] Index health check failed for #{class_current.name}: #{e.message}"
    false
  end
  ...
  def self.build(class_current, text_search, params, comparison_term = false)
    ...
    order_param = parse_order_param(params[:order])
    ...
    Rails.logger.error "[SearchService] Invalid ES query for #{class_current.name}: #{e.message}"
    SearchService.default_response
  end

  def self.build_light(class_current, text_search, params)
    return SearchService.default_response unless SearchService.exist_search?(class_current)

    text_search ||= "*"
    text_search = "*" if text_search == "" || text_search.nil?

    records = class_current.search(
      text_search,
      where: params[:where],
      order: params[:order],
      operator: params[:operator] || "and",
    ...
    {
      total_count: records&.total_count,
      records: records.hits.map { |hit| hit["_id"].to_i }
    }
  end
  ...
  def self.build_boolean_search(class_current, text_search, params, _comparison_term)
    ...
    query_string = {
      _source: false,
      from: (params[:page].to_i - 1) * (params[:limit] || 30),
    ...
    Rails.logger.error "[SearchService] Invalid ES query (boolean) for #{class_current.name}: #{e.message}"
    SearchService.default_response
  end
  ...
  def self.build_agg(params, text_search)
    ...
    if params[:entity_column_id]
      entity_columns = EntityColumn.find(params[:entity_column_id]).columns_view
      columns_in_use = entity_columns.map { |column| column["value"] }
      new_aggs = {}
      ...
      entity_columns.each do |column|
        new_aggs[column["text"].downcase.gsub(" ", "_").to_sym] = { limit: 50 } if column["value"] == "answers_cache"
      end
  ...
  def self.order_ids(class_current, ids, params = false)
    ...
    records = indexed_banks
      .where(id: ids)
      .order(Arel.sql("array_position(ARRAY[#{ids.join(',')}]::bigint[], #{class_current.table_name}.id)"))
      .to_a

    if class_current.respond_to?(:preload_display_associations)
      class_current.preload_display_associations(records)
    end
    if class_current.respond_to?(:preload_process_counts)
      class_current.preload_process_counts(records)
    end
    if class_current.respond_to?(:preload_field_requirement_data)
      class_current.preload_field_requirement_data(records)
    end
  ...
  def self.generate_order(params)
    return { created_at: { order: "desc" } } unless params[:order]

    order_params = params[:order]
    order_params = JSON.parse(order_params) if order_params.is_a?(String)

    return { created_at: { order: "desc" } } if order_params.blank?

    order_params.transform_values { |direction| { order: direction.to_s.downcase } }
  rescue JSON::ParserError
    { created_at: { order: "desc" } }
  end

  def self.parse_order_param(order)
    return nil unless order

    order_hash = order.is_a?(String) ? JSON.parse(order) : order
    return nil if order_hash.blank?

    order_hash.transform_keys(&:to_sym).transform_values { |direction| direction.to_s.downcase.to_sym }
  rescue JSON::ParserError
    nil
  end

  def self.search_v2(current_class, text_search, params)
    query = generate_search_query(params, text_search)
    current_class.search("*", body: query)
  end
end
