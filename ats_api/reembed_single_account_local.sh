...
# Script para reembedar candidatos e jobs de UMA conta específica (VERSÃO LOCAL - Docker)
# Use este script se quiser testar ou reembedar apenas uma conta localmente
...
if [ -z "$1" ]; then
  ...
  echo ""
  echo "Uso: ./reembed_single_account_local.sh <account_id>"
  echo ""
  echo "Para listar accounts disponíveis:"
  echo "  docker compose exec web bundle exec rails runner 'Account.all.each { |a| puts \"#{a.id}: #{a.name} (#{a.tenant})\" }'"
  echo ""
  exit 1
fi
...
echo "=========================================="
echo "🚀 REEMBEDDING ACCOUNT $ACCOUNT_ID (LOCAL)"
echo "=========================================="
echo ""
