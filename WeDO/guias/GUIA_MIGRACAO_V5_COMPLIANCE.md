...
| # | Problema | O que v5 já tem (incompleto) | O que LIA já tem (mesmo repo) | % | Gap real: o que falta + limpeza |
|---|----------|------------------------------|------------------------------|---|-------------------------------|
| P2 | Compliance opt-in | `DomainWorkflow._pre_check` aplica FairnessGuard automaticamente | Mesmo código | ~60% | O `DomainWorkflow` já aplica compliance para quem passa por ele. O gap são os caminhos que **bypessam**: endpoints REST diretos em `app/api/v1/` (ex: `job_vacancies.py`), Celery tasks em background, e o `ActionExecutorService` (Phase 1) que executa ações sem passar pelo workflow. Solução: interceptar esses caminhos com middleware FastAPI ou wrapping do ActionExecutor |
| P3 | 6/9 serviços faltam | FairnessGuard (3 layers), FactChecker, AuditService, PII Masking | Mesmo código | ~40% | Existem 4 dos 9 serviços de compliance. Faltam 5: **ExplainabilityService** (explicar ao candidato por que foi rejeitado — exigência LGPD Art.20), **RetentionPolicy** automática (hoje AuditService tem `retention_days` mas não executa purge), **ConsentManager** (rastrear consentimento do candidato para uso de dados), **ComplianceReporter** (dashboard de compliance para DPO), e **IncidentResponseService** (alertar quando compliance é violada). Os 4 existentes funcionam bem; os 5 faltantes são necessários para compliance completa LGPD/EU AI Act |
| P3.a | AuditCallback mutável | `ON CONFLICT DO UPDATE` em `libs/audit/lia_audit/audit_callback.py` | AuditService em `app/shared/compliance/audit_service.py` com retention | ~30% | O `AuditCallback` do LangChain usa `ON CONFLICT DO UPDATE` no PostgreSQL — isso permite sobrescrever registros de auditoria, o que viola SOX (imutabilidade de audit trail). O `AuditService` separado já tem retention policies mas usa o mesmo pattern. Solução: mudar para `INSERT` only + soft delete, ou usar append-only storage (ex: tabela particionada por mês, sem UPDATE/DELETE permissions) |
| P3.b | PII Stripping parcial | `install_global_pii_masking()` no `app/main.py` + `strip_pii_for_llm_prompt` | Mesmo código | ~70% | O masking global funciona nos **logs** (todos os `logging.*` passam pelo filtro PII). O `strip_pii_for_llm_prompt` existe e é usado em `rubric_evaluation_service.py` (linha 894) e `wsi_interview_graph.py` (linha 534). Gap: **não é chamado** antes de enviar dados ao LLM em `job_management`, `messaging`, `sourcing`, `analytics` e `insights`. Solução: adicionar `strip_pii_for_llm_prompt` no `LangGraphReActBase` antes de montar o prompt, garantindo cobertura automática em todos os agentes |
| P3.c | FactChecker só local | FactChecker no `DomainWorkflow._post_check` | Mesmo código | ~50% | O FactChecker **já é global** via `_post_check` no DomainWorkflow — todo domínio que passa pelo workflow recebe validação. O gap é duplo: (1) domínios que bypessam o workflow (mesmos de P2), e (2) o FactChecker valida claims genéricos (números, datas, percentuais) mas **não tem validadores específicos** por domínio (ex: verificar se um salário citado bate com a faixa da vaga, ou se um candidato realmente está na fase mencionada) |
| P4 | Serviços acoplados | FairnessGuard importado direto em `sourcing_react_agent.py`, `policy_react_agent.py`, `pipeline_tool_registry.py` | `FairnessGuardMiddleware` em `app/shared/compliance/` | ~50% | O middleware existe mas **não é usado consistentemente**. Sourcing importa `FairnessGuard` diretamente (linha 151 do agent), Pipeline chama na rejeição (linha 513 do tool_registry), Policy chama no agent. Cada um faz de um jeito diferente. Solução: remover importações diretas, fazer `ComplianceDomainPrompt` chamar o middleware automaticamente em `pre_process` e `post_process`, eliminando a necessidade de cada domínio lembrar de chamar. **Limpeza:** deletar `jobs/fairness.py`, `evaluation/security.py`, `sourcing/fairness.py`, `sourcing/fact_checker.py` e remover imports diretos em cada `domain.py` (ver Seção 4.4) |
| P4.a | Implementações divergem | Sourcing usa Layer 3 (output check), Pipeline usa `check_rejection_reason`, Policy usa `check` básico | `DomainWorkflow` padroniza | ~60% | As implementações locais já **divergiram** do FairnessGuard original. Sourcing adicionou Layer 3 (semântica no output) que outros domínios não têm. Pipeline tem `check_rejection_reason` (verifica motivo de rejeição para bias) que é exclusivo dele. Quando um fix é feito no FairnessGuard central, as versões locais **não recebem** o fix. Solução: centralizar todas as variantes no middleware e parametrizar por domínio (ex: `sourcing` ativa Layer 3, `pipeline` ativa rejection check). **Incremento:** o FairnessGuard central precisa absorver as features que hoje são locais (Layer 3 do Sourcing, rejection check do Pipeline) antes de deletar as cópias locais |
| P5 | Pipeline errado | Compliance roda em `_pre_check` (antes) e `_post_check` (depois) | Mesmo código | ~50% | O FairnessGuard roda **antes** do processamento (query do usuário) e o FactChecker roda **depois** (resposta gerada). O gap: **durante** o processamento, o agente ReAct faz tool calls que podem gerar filtros discriminatórios (ex: LLM gera `WHERE age < 30` numa query SQL). Nenhum check acontece nesse ponto intermediário. Solução: adicionar FairnessGuard no `TimedToolNode` (já existe em `langgraph_react_base.py`) para interceptar inputs/outputs de cada tool call |
| P5.a | PII vai ao LLM | `strip_pii_for_llm_prompt` em `cv_screening` e `wsi_interview` | Mesmo código | ~40% | Só 2 de 9 domínios usam stripping antes de enviar ao LLM. Nos outros 7 (jobs, messaging, sourcing, analytics, automation, hiring_policy, ats_integration), dados como nome completo, email, telefone e CPF podem ir como contexto para o LLM. Impacto LGPD: dados pessoais em API de terceiros (Anthropic/Google) sem base legal. Solução: `strip_pii_for_llm_prompt` no `LangGraphReActBase._build_messages()` automaticamente |
| P5.b | FairnessGuard só na query | `_pre_check` analisa a mensagem do usuário | Layer 3 no `sourcing_react_agent.py` analisa output do LLM | ~30% | O FairnessGuard Layer 1 (regex) e Layer 2 (implicit bias) rodam na **query**. Layer 3 (semântica) analisa o **output** — mas só existe no Sourcing. Cenário de risco: recrutador pede "candidatos para startup dinâmica" (passa Layer 1/2), LLM gera filtro `age_range: 25-35` (discriminação etária) que **nenhum check intercepta** nos domínios sem Layer 3. Solução: ativar Layer 3 em todos os domínios via middleware, não só Sourcing |
| P5.c | FactChecker só sourcing | `_post_check` no DomainWorkflow aplica FactChecker | Mesmo código | ~60% | O FactChecker valida claims numéricos (salários, contagens, datas) globalmente. O gap: validadores **domain-specific** não existem. Ex: Evaluation diz "candidato pontuou 85/100" mas o score real é 72. Analytics diz "tempo médio de contratação: 15 dias" mas o cálculo real dá 23. O FactChecker genérico não consegue verificar isso sem acesso aos dados do domínio. Solução: cada domínio registra validadores específicos no FactChecker (ex: `evaluation` registra `validate_score`, `analytics` registra `validate_metric`) |
| P6 | Sem camada intermediária | `DomainPrompt` em `app/domains/base.py` é base mínima (só interface) | `DomainWorkflow` em `app/domains/workflow.py` é camada de execução | ~40% | Existem duas "camadas" mas desconectadas: `DomainPrompt` define a interface (process_intent, execute_action) e `DomainWorkflow` orquestra (pre_check → execute → post_check). O gap: não existe uma classe entre elas que **garanta** compliance. Um novo domínio pode herdar `DomainPrompt` e implementar `execute_action` sem nenhum check. Solução: criar `ComplianceDomainPrompt(DomainPrompt)` que herda a interface e adiciona compliance obrigatória; `DomainWorkflow` passa a usar só `ComplianceDomainPrompt` |
| P7 | Novos domínios sem compliance | `DomainPrompt` base não inclui compliance | — | 0% | Hoje para criar um novo domínio: `class MeuDominio(DomainPrompt)`. Nada obriga a adicionar FairnessGuard, PII, Audit. O desenvolvedor precisa **saber** que precisa e **lembrar** de importar cada serviço. Com `ComplianceDomainPrompt`: `class MeuDominio(ComplianceDomainPrompt)` — compliance vem automaticamente. É a diferença entre opt-in (falha) e opt-out (seguro por default) |

#### Qualidade de Resposta (Caminho 3)

| # | Problema | O que v5 já tem | O que LIA já tem (mesmo repo) | % | Gap real |
|---|----------|----------------|------------------------------|---|----------|
| P8 | Flat → ReAct | `ActionExecutorService` em `app/orchestrator/action_executor.py` com dicionário `ACTIONABLE_INTENTS` (linhas 28-247) e if/elif (linhas 374-415) | `LangGraphReActBase` em `libs/agents-core/` + 9 agentes ReAct implementados (Jobs, Screening, Sourcing, Policy, Recruiter×2, Communication, Analytics, ATS) | ~80% | O `DomainWorkflow` já tenta ReAct primeiro (`_try_react_agent`, linha 359) e cai no Flat se não encontrar agente. O gap: verificar quais domínios não têm agente ReAct registrado no `DomainRegistry` e criar os faltantes seguindo o padrão canônico de 4 arquivos (agent, system_prompt, tool_registry, stage_context). O `ActionExecutorService` (Phase 1) pode ser mantido como fast-path para intents triviais |
| P8.a | Sem cross-domain | Cada agente processa isolado no seu domínio | `StateManager` em `app/orchestrator/state_manager.py` persiste resultados cross-agente | ~60% | O `MainOrchestrator` processa um domínio por vez. Fluxos como "aplique os 5 melhores candidatos → agende entrevistas → configure avaliação" exigem 3 domínios em sequência (screening→scheduling→evaluation). Hoje o recrutador precisa dar 3 comandos separados. Solução: criar `MultiDomainPlan` que decompõe a intenção em steps e usa StateManager para passar resultados entre domínios automaticamente |
| P9 | Keyword frágil | `FastRouter` em `app/orchestrator/fast_router.py` com regex por domínio (Tier 4 do CascadedRouter) | `CascadedRouter` em `app/orchestrator/cascaded_router.py` com 6 tiers: Cache LRU → Redis → VectorCache (pgvector) → FastRouter → LLMCascade (Haiku→Sonnet→Opus) → Clarification | ~70% | O routing por LLM **já existe** como Tier 5 (LLMCascade). O problema é que o FastRouter (Tier 4) intercepta **antes** com matches errados. Ex: "comparar candidatos" bate em 3 domínios (analytics, screening, sourcing) via regex. Solução: (1) remover patterns ambíguos do FastRouter, (2) subir threshold de confiança do FastRouter para reduzir false positives, ou (3) inverter prioridade (LLM primeiro, FastRouter como fallback para latência) |
| P9.a | Colisão keywords | `_KEYWORD_ACTION_MAP` em cada `domain.py` — `analytics/domain.py` (linhas 11-113), `interview_scheduling/domain.py` (linhas 19-128), etc. | LLMCascade com `INTENT_CLASSIFICATION_EXAMPLES` em `few_shot_examples.py` | ~70% | Após o CascadedRouter escolher o domínio, o domínio usa seu próprio `_KEYWORD_ACTION_MAP` para decidir a **ação**. Aqui a colisão é intra-domínio: "agendar" pode ser `schedule_interview` ou `schedule_report` dependendo do contexto. Quando o domínio migra para ReAct (P8), o agente LLM decide a ação por raciocínio, eliminando esses maps. Gap diminui automaticamente com a resolução de P8 |
| P9.b | Não entende negação | Regex no FastRouter não diferencia "mude o salário" de "NÃO mude o salário" | `NEGATION_DETECTION_BLOCK` em `app/shared/prompts/interaction_patterns.py` já existe como bloco injetável | ~80% | O bloco de detecção de negação existe e funciona. O gap: domínios que ainda usam `_KEYWORD_ACTION_MAP` (Flat) **não passam pelo LLM** onde o bloco seria aplicado. Quando esses domínios migram para ReAct (P8), o bloco é automaticamente injetado no system prompt. Portanto P9.b **se resolve junto com P8** — não precisa de trabalho separado |
| P9.c | Linguagem natural | Regex faz match literal: "agendar entrevista" funciona, "deixar pra outro dia" não | LLMCascade usa classificação por LLM + `INTENT_CLASSIFICATION_EXAMPLES` com few-shot para cobertura de linguagem coloquial | ~70% | O LLMCascade já lida com linguagem natural. Gap: os few-shot examples cobrem job, intent e salary mas **não cobrem** cenários de scheduling ("deixar pra outro dia"), communication ("manda um zap"), ou analytics ("como tá o funil"). Solução: expandir `INTENT_CLASSIFICATION_EXAMPLES` com 3-5 exemplos por domínio, incluindo variações coloquiais em PT-BR |
| P9.d | Referências temporais | "Ontem", "semana passada", "mês que vem" ignoradas pelo regex | MemoryResolver em `app/orchestrator/memory_resolver.py` resolve referências a entidades (candidatos, vagas) mas **não resolve tempo** | ~30% | Nenhuma parte do sistema converte referências temporais relativas em datas absolutas. "Entrevistas de ontem" deveria virar `date=2026-03-31` mas o regex ignora e o MemoryResolver só resolve entidades. Solução: implementar `TemporalResolver` (regex simples: "ontem"→`-1d`, "semana passada"→`-7d`, etc.) e integrá-lo ao MemoryResolver como step adicional antes do routing |
| P10 | Sem memória | Flat handlers recebem `conversation_history` mas **não persistem estado entre turnos** — cada mensagem é processada isoladamente | `WorkingMemoryService` em `libs/agents-core/` persiste `collected_fields`, `current_plan`, `pending_actions` no DB por session+domain | ~70% | Agentes ReAct já usam WorkingMemory automaticamente (herdado de `LangGraphReActBase`). Domínios Flat processam cada mensagem como se fosse a primeira. Quando migram para ReAct (P8), **herdam WorkingMemory automaticamente**. Gap se resolve junto com P8 |
| P10.a | Sem StageContext | `cv_screening` e `job_management` têm StageContext. `analytics`, `communication`, `automation`, `ats_integration` **não têm** | Pattern `StageContext` disponível em `app/domains/recruiter_assistant/agents/jobs_mgmt_stage_context.py` como referência | ~60% | StageContext adapta o system prompt e as tools disponíveis conforme a "fase" do trabalho (ex: triage→screening→interview→offer). Domínios sem ele usam o mesmo prompt independente do contexto. Solução: criar StageContext para analytics (dashboard→drill-down→export), communication (template→personalizar→enviar), e automation (configurar→testar→ativar). O pattern já está documentado — replicar para os domínios faltantes |
| P10.b | Sem anáforas | — | MemoryResolver resolve "aquela vaga" → `job_id=123`, "o candidato" → `candidate_id=456`, "o segundo" → índice da lista mostrada | ~80% | Funciona bem para referências a **entidades** (candidatos, vagas, entrevistas). Gap: não resolve referências a **ações anteriores** ("faz de novo", "repete", "desfaz isso") nem a **resultados** ("aquele relatório", "o score que você deu"). Solução: expandir o MemoryResolver com `action_history` e `result_cache` no WorkingMemory |
| P10.c | Sem cross-session | Cada sessão começa do zero — `session_id` novo = memória vazia | WorkingMemory persiste por session+domain. `ConversationMemory` (referenciado em `orchestrator.py`) gera resumos periódicos | ~50% | O ConversationMemory existe mas **gera resumos genéricos**. Quando o recrutador volta no dia seguinte, o agente não sabe "ontem trabalhamos na vaga de Dev Senior e filtramos 12 candidatos". Solução: implementar `SessionBridge` que ao iniciar nova sessão, carrega o resumo da última sessão do mesmo usuário+domínio e injeta como contexto inicial |
| P11 | Prompts estáticos | `MASTER_ORCHESTRATOR_PROMPT` e `STAGE_SPECIALIZED_PROMPTS` hardcoded em `libs/agents-core/lia_agents_core/nodes.py` | `PromptRegistry` em `app/shared/prompts/prompt_registry.py` + `PromptLoader` em `app/shared/prompts/loader.py` + 10 YAMLs de domínio em `app/prompts/domains/` + 5 compartilhados em `app/prompts/shared/` | ~75% | Os YAMLs existem e são completos (persona, scope, rules, behavioral_rules por domínio). O PromptLoader carrega e faz cache. O gap: os agentes v5 **não chamam** o PromptLoader — eles usam as strings hardcoded diretamente. Solução: em cada `*_system_prompt.py`, substituir a string fixa por `PromptLoader.load("domains/{domain}.yaml")` + composição com blocos shared. Trabalho mecânico, ~30min por domínio |
| P11.a | Sem BARS | — | `RubricEvaluationService` em `app/domains/cv_screening/services/rubric_evaluation_service.py` com escala BARS: EXCEEDS(100), MEETS(75), PARTIAL(40), MISSING(0). Modelo em `libs/models/lia_models/rubric.py` | ~80% | BARS funciona completo no cv_screening. Gap: evaluation, hiring_policy e sourcing fazem avaliações **sem escala padronizada** — cada um usa critérios ad-hoc. Solução: generalizar `RubricEvaluationService` para aceitar rubrics por domínio (não só screening) e registrar rubrics específicas (policy compliance, sourcing quality, interview readiness) |
| P11.b | Sem few-shot | — | `few_shot_examples.py` em `app/shared/prompts/` com categorias: JOB_EXTRACTION, INTENT_CLASSIFICATION, SALARY_ANALYSIS | ~60% | 3 categorias de few-shot existem. Faltam: CANDIDATE_EVALUATION (bom vs ruim parecer), SCHEDULING_NEGOTIATION (agendar com restrições), COMMUNICATION_TONE (formal vs informal), ANALYTICS_QUERY (como pedir dados), SOURCING_QUERY (busca com filtros complexos). Solução: adicionar 3-5 exemplos por categoria faltante, seguindo o formato existente em `few_shot_examples.py` |
| P11.c | Sem blocos composíveis | — | `ANTI_SYCOPHANCY_BLOCK`, `CHAIN_OF_THOUGHT_BLOCK`, `NEGATION_DETECTION_BLOCK`, `DEFENSIVE_BLOCK` em `app/shared/prompts/interaction_patterns.py` | ~80% | Os blocos existem e funcionam. Gap: **não são injetados automaticamente** — cada system_prompt precisa importá-los manualmente. Solução: o `ComplianceDomainPrompt` (P6/P7) pode injetar blocos obrigatórios (ANTI_SYCOPHANCY, DEFENSIVE) automaticamente ao montar o prompt, e blocos opcionais (CoT) por configuração do domínio |
| P11.d | Sem A/B testing | — | `PromptRegistry.compare_versions()` compara prompts lado a lado. Versionamento semântico (X.Y.Z) por prompt | ~40% | Infraestrutura de versionamento existe. Gap: **não existe runner de split test** que direcione X% do tráfego para prompt v1.2 e Y% para v1.3, meça resultados (satisfação, accuracy, tempo de resposta) e declare vencedor. Solução: implementar `PromptExperiment` que usa o PromptRegistry para selecionar versão por session_id hash, loggar resultados no AuditService, e gerar relatório de comparação |
| P11.e | YAMLs desconectados | v5 carrega prompts de strings Python, ignora os YAMLs | `PromptLoader` + 10 YAMLs completos (persona, scope_in, scope_out, behavioral_rules, system_prompt por domínio) | ~90% | Os YAMLs estão prontos e o loader funciona. É o gap mais fácil de fechar: em cada agente, trocar `SYSTEM_PROMPT = "..."` por `SYSTEM_PROMPT = PromptLoader.load("domains/sourcing.yaml").system_prompt`. Literalmente 1 linha por agente. O bloqueio não é técnico — é que ninguém fez |
| P11.f | Sem persona | Prompts v5 não definem quem é a LIA — tom genérico de chatbot | `lia_persona.yaml` define identidade (profissional, empática, direta), `hr_vocabulary.yaml` padroniza termos (CLT, Fit Cultural, Shortlist), `ethical_guidelines.yaml` proíbe bias | ~90% | Mesmo caso de P11.e: os YAMLs de persona existem, o loader funciona, mas os agentes v5 não carregam. Solução idêntica: adicionar `persona = PromptLoader.load("shared/lia_persona.yaml")` na composição do prompt. Bônus: `hr_vocabulary.yaml` garante que a LIA use termos corretos de RH em PT-BR em vez de traduzir literalmente do inglês |
| P12 | Gap de tools | `ToolRegistry` em `app/tools/registry.py` + metadata YAML em `app/tools/tool_registry_metadata.yaml` + `tool_definition_to_langchain_tool` conversor | Mesmo código + tools implementadas para Jobs (`job_wizard_tools.py`), Screening (`candidate_tools.py`), Communication (`communication_tools.py`), Analytics (`analytics_query_tools.py`) | ~55% | 4 domínios têm tools reais. 5 domínios têm tools parciais ou stubs: `messaging` (templates sem personalização), `insights` (sem aggregation cross-domain), `scheduling` (ações básicas, sem negociação), `automation` (triggers sem execução real), `ats_integration` (sync declarado, import parcial). Solução: implementar handlers faltantes domínio por domínio, seguindo o padrão de `job_wizard_tools.py` como referência |
| P12.a | Stubs sem implementação | Ações declaradas no YAML metadata sem handler Python correspondente | Tools reais com DB queries, validação, e integração com serviços externos (Apify no Sourcing, SMTP no Communication) | ~50% | O YAML metadata declara ações como `enrich_candidate_profile`, `generate_diversity_report`, `auto_schedule_batch` que não têm handler implementado. O agente ReAct vê a tool disponível, tenta chamar, e recebe erro ou resposta vazia. Solução: implementar cada handler faltante — o pattern é consistente: async function que recebe `ToolExecutionContext`, faz query/operação, retorna resultado tipado |
| P12.b | Sem agent-level registry | Domínios que ainda usam Flat não expõem tools para o agente — a lógica está hardcoded no `execute_action` | Domínios ReAct têm `tool_registry.py` próprio (ex: `pipeline_tool_registry.py` com `move_candidate`, `analyze_cv`, `schedule_interview`) | ~60% | Quando um domínio migra de Flat para ReAct (P8), ele **precisa** de um `tool_registry.py` que liste as tools disponíveis para o agente. Domínios sem: analytics (usa execute_action direto), interview_scheduling (hybrid), automation (triggers sem registry). Solução: extrair as ações do `execute_action` de cada domínio e registrá-las como tools no formato `ToolDefinition` |
| P12.c | Sem cross-domain access | Tools confinadas ao domínio — screening não acessa tools de sourcing, analytics não acessa tools de evaluation | `app/shared/tools/` tem `export_tools.py`, `predictive_tools.py`, `proactive_tools.py` compartilhadas | ~40% | O agente de screening quer comparar candidatos usando WSI scores (evaluation) mas não tem acesso à tool. O agente de analytics quer dados de pipeline (screening) + vagas (jobs) mas só acessa suas próprias tools. Shared tools existem mas são poucas. Solução: criar `GlobalToolRegistry` que expõe tools read-only cross-domain (ex: screening pode **ler** scores de evaluation, mas não pode **alterar**). Write operations permanecem restritas ao domínio dono |
| P13 | Sem batch | 1 item por vez — nenhuma API aceita lista | — | 0% | Não existe nenhuma infraestrutura de batch no codebase. Cada operação (avaliar candidato, enviar mensagem, mover na pipeline) processa 1 item. Para "avalie os 50 candidatos da vaga Dev Senior", o recrutador precisa repetir 50 vezes. Solução: implementar `BatchService` com `asyncio.Semaphore` para concorrência controlada (ex: 5 simultâneos), fila de retry, e progress tracking via WebSocket |
| P13.a | Sem paralelo | Processamento sequencial — 50 candidatos = 50×latência | — | 0% | Mesmo dentro de um domínio, operações são sequenciais. `for candidate in candidates: await process(candidate)`. Solução: `asyncio.gather` com Semaphore para limitar concorrência, wrapping de cada operação com timeout e retry. O `TimedToolNode` já tem timeout por tool — basta expandir para batch de tools |
| P13.b | N chamadas manuais | Usuário precisa pedir N vezes pelo chat | — | 0% | Não existe endpoint REST ou comando de chat que aceite lista. "Mova os candidatos aprovados para entrevista" deveria aceitar `candidate_ids: [1,2,3,...50]` mas hoje aceita só `candidate_id: 1`. Solução: criar variantes batch das tools existentes (ex: `move_candidates_batch`) e ensinar o agente ReAct a usá-las quando detectar intenção de operação em lote |
