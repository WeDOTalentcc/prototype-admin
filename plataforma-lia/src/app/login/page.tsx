...
export default function LoginPage() {
  ...
  const handlePasswordSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : String(err) || "Senha incorreta. Verifique suas credenciais.")
    } finally {
      ...
      <div className="w-full lg:w-1/2 flex flex-col relative z-10">
        ...
        <div className="w-full max-w-md bg-white/90 dark:bg-lia-bg-secondary/90 backdrop-blur-md rounded-xl shadow-2xl p-8 lg:p-10">
          ...
          <div className="mb-6 text-center">
            <h2 className="text-xl font-semibold lia-text-950">
          ...
          {error && (
            <div
              id="login-error"
              role="alert"
              className="mb-4 p-3 bg-status-error/10 border border-status-error/30 rounded-xl flex items-start gap-2"
            >
          ...
          {/* ── ETAPA 1: EMAIL ── */}
          {step === "email" && (
            <form onSubmit={handleEmailStep} className="space-y-4" aria-label="Formulário de acesso — etapa email">
              <div>
                <label htmlFor="campo-email" className="block text-xs font-medium lia-text-800 mb-1.5">
                  Email
                </label>
          ...
          {/* ── ETAPA 2: SENHA ── */}
          {step === "password" && (
            <form onSubmit={handlePasswordSubmit} className="space-y-4" aria-label="Formulário de acesso — etapa senha">
              {/* Email confirmado */}
              <div className="flex items-center justify-between px-3 py-2.5 bg-gray-50 dark:bg-lia-bg-elevated border border-lia-border-subtle rounded-xl">
                <div className="flex items-center gap-2 min-w-0">
                  <Mail className="w-4 h-4 lia-text-400 shrink-0" aria-hidden="true" />
                  <span className="text-sm lia-text-700 truncate">{email}</span>
                </div>
                <button
                  type="button"
                  onClick={handleBack}
              ...
              {/* Senha */}
              <div>
                <label htmlFor="campo-senha" className="block text-xs font-medium lia-text-800 mb-1.5">
                  Senha
                </label>
                <div className="relative">
                  ...
                  <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 lia-text-400 hover:lia-text-600"
              ...
              <div className="flex items-center justify-between text-xs">
                <label htmlFor="campo-lembrar" className="flex items-center gap-2 cursor-pointer">
                  <input
                    id="campo-lembrar"
                    type="checkbox"
                    checked={rememberMe}
