...
function OpinionSkeleton() {
  return (
    <div className="space-y-3">
      {[1, 2].map((i) => (
        <div key={i} className="bg-white dark:bg-lia-bg-secondary border border-lia-border-subtle rounded-md p-4 animate-pulse motion-reduce:animate-none">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-8 h-8 bg-gray-200 dark:bg-lia-bg-elevated rounded-full" />
            <div className="flex-1">
              <div className="w-32 h-4 bg-gray-200 dark:bg-lia-bg-elevated rounded-md mb-1" />
              <div className="w-24 h-3 bg-gray-200 dark:bg-lia-bg-elevated rounded-md" />
            </div>
          </div>
          <div className="space-y-2">
            <div className="w-full h-3 bg-gray-200 dark:bg-lia-bg-elevated rounded-md" />
            <div className="w-3/4 h-3 bg-gray-200 dark:bg-lia-bg-elevated rounded-md" />
          </div>
        </div>
      ))}
    </div>
          ...
          <div className="flex items-center gap-4 border-b border-lia-border-subtle pb-3">
            <button
              ...
              className={`flex items-center gap-2 pb-2 text-sm font-medium transition-colors motion-reduce:transition-none ${
                opinionsSubTab === "pareceres"
                  ? "text-lia-text-secondary border-b-2 border-gray-900 dark:border-gray-50"
            ...
            >
              ...
              {opinionsHistory.length > 0 && (
                <Badge className="text-xs px-1.5 py-0 h-4 bg-gray-900 text-white">{opinionsHistory.length}</Badge>
              )}
            </button>
            <button
          ...
          {opinionsSubTab === "pareceres" && (
            <>
              ...
              {!isLoadingOpinions && opinionsHistory.length === 0 && (
                <div className="bg-white dark:bg-lia-bg-secondary border border-lia-border-subtle rounded-md p-8 text-center">
                  ...
                  {opinionsHistory.map((opinion) => {
                    ...
                    return (
                      <div key={opinion.id as string} className="relative">
                        {!opinion.is_current && (
                          <Badge className="absolute top-2 right-2 text-xs px-1.5 py-0 h-4 bg-gray-100 text-lia-text-secondary z-10">
                            v{opinion.version as number} - Histórico
                          </Badge>
                        )}
                        <div className="bg-white dark:bg-lia-bg-secondary border border-lia-border-subtle rounded-md overflow-hidden">
                          <button
              ...
              {!isLoadingAnalyses && analyses.length > 0 && (
                <div className="space-y-3">
                  {analyses.map((analysis) => {
                    ...
                    return (
                      <div key={analysis.id as string} className="bg-white dark:bg-lia-bg-secondary border border-lia-border-subtle rounded-md overflow-hidden">
                        <button
