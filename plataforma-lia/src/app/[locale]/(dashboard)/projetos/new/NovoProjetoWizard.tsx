...
function Step3Template({
  ...
  const { persona: _ap3 } = useAiPersona()
  const personaName = _ap3?.name ?? "IA"
    ...
    <div className="space-y-4">
      ...
      {mode === "Gerar com IA" && (
        <div className="flex flex-col items-center justify-center py-10 gap-3 text-center">
          ...
          <p className="text-sm text-lia-text-secondary">
            {`${personaName} gerará um pipeline otimizado com base na vaga selecionada.`}
          </p>
          <button
            type="button"
          ...
          >
            {data.templateKey === "ai_generated" ? "✓ Selecionado" : `Gerar pipeline com ${personaName}`}
          </button>
        </div>
      )}
    </div>
