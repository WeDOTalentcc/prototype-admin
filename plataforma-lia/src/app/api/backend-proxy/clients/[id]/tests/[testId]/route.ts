...
export async function GET(
  ...
  try {
    ...
    const paramValidation = validateParams({ id, testId }, routeParamsSchema)
    if (!paramValidation.success) return paramValidation.response
    
    const backendUrl = `${BACKEND_URL}/api/v1/clients/${id}/tests/${testId}`
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'Erro ao buscar teste do cliente', details: errorData },
...
export async function PUT(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}))
      return NextResponse.json(
        { error: 'Erro ao atualizar teste do cliente', details: errorData },
        { status: response.status }
      )
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}))
      return NextResponse.json(
        { error: 'Erro ao excluir teste do cliente', details: errorData },
        { status: response.status }
      )
    }
