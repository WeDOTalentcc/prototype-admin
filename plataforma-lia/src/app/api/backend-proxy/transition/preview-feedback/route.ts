...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/v1/recruitment-stages/transition/preview-feedback`, {
      method: 'POST',
      headers: getAuthHeaders(request),
      body: JSON.stringify(bodyResult.data),
    })
    if (!response.ok) {
      return NextResponse.json({ error: 'Failed to preview feedback' }, { status: response.status })
    }
