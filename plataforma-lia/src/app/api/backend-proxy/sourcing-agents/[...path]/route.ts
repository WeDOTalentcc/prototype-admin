...
function buildApiPath(path: string[]): string {
  return `/api/v1/sourcing-agents/${path.join("/")}`
}
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  const { path } = await params
  const body = await req.text()
  const res = await fetch(`${BACKEND_URL}${buildApiPath(path)}`, { method: "PATCH", headers: getAuthHeaders(req), body })
  return new NextResponse(await res.text(), { status: res.status, headers: { "Content-Type": "application/json" } })
}
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  const { path } = await params
  const body = await req.text()
  const res = await fetch(`${BACKEND_URL}${buildApiPath(path)}`, { method: "PUT", headers: getAuthHeaders(req), body })
  return new NextResponse(await res.text(), { status: res.status, headers: { "Content-Type": "application/json" } })
}
