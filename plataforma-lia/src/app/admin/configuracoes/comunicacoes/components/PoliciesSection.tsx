...
const policyTypeLabels: Record<string, { label: string; color: string }> = {
  rate_limit: { label: 'Rate Limit', color: 'bg-lia-bg-tertiary dark:bg-lia-bg-secondary text-lia-text-secondary dark:text-lia-text-tertiary' },
    ...
    <div className="space-y-4">
      ...
      {policiesLoading ? (
        ...
        <Card className="flex flex-col items-center justify-center py-12">
          <Shield className="w-12 h-12 mb-3 text-lia-text-tertiary dark:text-lia-text-secondary"  />
        ...
        <>
            ...
            <Card className="mb-4">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-xs font-medium text-lia-text-secondary dark:text-lia-text-tertiary" >Tipo</label>
                    <select
                ...
                <div>
                  <label className="text-xs font-medium text-lia-text-secondary dark:text-lia-text-tertiary" >Escopo</label>
                  <select
                    className="w-full mt-1 px-3 py-2 border rounded-md text-sm"
          ...
          {Object.entries(groupedPolicies).map(([type, typePolicies]) => (
            <div key={type} className="space-y-3">
              <div className="flex items-center gap-2">
                <Badge className={`text-xs ${policyTypeLabels[type]?.color || 'bg-lia-bg-tertiary text-lia-text-secondary'}`}>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <Shield className={`w-4 h-4 ${policy.isActive ? 'text-lia-text-primary' : 'text-lia-text-tertiary dark:text-lia-text-secondary'}`} />
                            <h4 className="font-medium text-sm text-lia-text-primary dark:text-lia-text-primary" >
                              {policy.name}
                            </h4>
                            <Badge 
                              variant="outline" 
                              className={`text-xs ${policy.scope === 'platform' ? 'bg-wedo-purple/10 text-wedo-purple border-wedo-purple/30' : 'bg-lia-bg-secondary text-lia-text-secondary border-lia-border-subtle'}`}
                            >
