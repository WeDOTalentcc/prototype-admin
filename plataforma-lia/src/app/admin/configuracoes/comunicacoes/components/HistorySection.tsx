...
const getStatusBadge = (status: CommunicationLogEntry['status']) => {
  const styles: Record<string, string> = {
    sent: 'bg-status-warning/15 text-status-warning dark:bg-status-warning/30 dark:text-status-warning',
    delivered: 'bg-status-success/15 text-status-success dark:bg-status-success/30 dark:text-status-success',
...
const getChannelIcon = (channel: CommunicationLogEntry['channel']) => {
  switch (channel) {
    ...
    case 'sms':
      return <MessageSquare className="w-4 h-4 text-wedo-purple" />
    default:
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-4 pt-4 border-t border-lia-border-subtle dark:border-lia-border-subtle" >
              <Button
                size="sm"
                variant="outline"
