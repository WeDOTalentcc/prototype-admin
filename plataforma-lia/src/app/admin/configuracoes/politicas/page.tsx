...
const CATEGORY_CONFIG: Record<PolicyCategory, { label: string; icon: React.ElementType; color: string; description: string }> = {
  data_retention: {
    ...
    icon: Database,
    color: 'bg-lia-bg-tertiary dark:bg-lia-bg-secondary text-lia-text-secondary dark:text-lia-text-tertiary',
...
export default function GlobalPoliciesPage() {
  ...
  if (isLoading && policies.length === 0) {
    return (
      <div role="status" aria-live="polite" aria-busy={true} className="min-h-screen bg-lia-bg-primary dark:bg-lia-bg-primary flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lia-text-secondary">Carregando políticas...</p>
        </div>
      </div>
    )
  }
  ...
  if (error && policies.length === 0) {
    return (
      <div className="min-h-screen bg-lia-bg-primary dark:bg-lia-bg-primary flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lia-text-secondary mb-4">{error.message}</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Card className="mb-6 border-lia-border-subtle dark:border-lia-border-subtle">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium text-lia-text-primary dark:text-lia-text-primary flex items-center gap-2">
          ...
          <CardContent>
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Button
                ...
                className="bg-lia-btn-primary-bg hover:bg-lia-btn-primary-hover text-lia-btn-primary-text dark:bg-lia-bg-secondary dark:text-lia-text-primary dark:hover:bg-lia-interactive-active gap-2 sm:w-auto"
              >
                {isApplyingSector ? (
                  <Loader2 className="w-4 h-4 animate-spin motion-reduce:animate-none" />
                ) : (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {(Object.entries(CATEGORY_CONFIG) as [PolicyCategory, typeof CATEGORY_CONFIG[PolicyCategory]][]).map(([key, config]) => {
            ...
            return (
              <Card
                key={key}
                className={`cursor-pointer transition-colors motion-reduce:transition-none ${activeTab === key ? 'ring-2 ring-lia-btn-primary-bg/20' : ''}`}
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as PolicyCategory | 'all')} className="space-y-6">
          <TabsList className="bg-lia-bg-tertiary dark:bg-lia-bg-secondary">
            ...
            {filteredPolicies.length === 0 ? (
              <Card className="border-dashed">
                <CardContent className="py-12">
                  <div className="text-center">
                    <FileText className="w-12 h-12 text-lia-text-tertiary mx-auto mb-4" />
                    <h3 className="text-lg font-medium text-lia-text-primary dark:text-lia-text-primary mb-2">
              ...
              <div className="bg-lia-bg-primary dark:bg-lia-bg-secondary rounded-md border border-lia-border-subtle dark:border-lia-border-subtle overflow-hidden">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-lia-border-subtle dark:border-lia-border-subtle bg-lia-bg-secondary dark:bg-lia-bg-secondary/50">
                        ...
                        <th className="text-right text-xs font-medium text-lia-text-secondary uppercase tracking-wider px-6 py-3">
                          Ações
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-lia-border-subtle dark:divide-lia-border-strong">
                      {filteredPolicies.map((policy) => {
                        ...
                        return (
                          <tr 
                            key={policy.id}
                            className="hover:bg-lia-bg-secondary dark:hover:bg-lia-btn-primary-hover/50 transition-colors motion-reduce:transition-none"
                          >
                            <td className="px-6 py-4">
                              <div>
                                ...
                                <p className="text-xs text-lia-text-secondary dark:text-lia-text-tertiary mt-1 max-w-md">
                                  {policy.description}
                                </p>
                              </div>
                            </td>
                            ...
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-2">
                                <Switch
                                  ...
                                  disabled={isUpdating}
                                />
                                <span className={`text-xs ${policy.isActive ? 'text-status-success' : 'text-lia-text-secondary'}`}>
                ...
                <div>
                  ...
                  {editingPolicy.valueType === 'number' && (
                    <div className="flex items-center gap-2">
                      ...
                      />
                      {editingPolicy.unit && (
                        <span className="text-sm text-lia-text-secondary min-w-20">
                          {editingPolicy.unit}
                        </span>
                      )}
                    </div>
            ...
            <div className="space-y-4 py-4">
              {history.length === 0 ? (
                <div className="text-center py-8">
                  <History className="w-12 h-12 text-lia-text-tertiary mx-auto mb-4" />
                  <p className="text-lia-text-secondary">Nenhum histórico de alterações encontrado</p>
                </div>
              ) : (
                history.map((entry) => (
                  <div 
                    key={entry.id}
                    className="p-4 rounded-md border border-lia-border-subtle dark:border-lia-border-subtle bg-lia-bg-secondary dark:bg-lia-bg-secondary/50"
                  >
