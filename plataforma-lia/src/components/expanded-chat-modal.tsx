...
import { WizardContainer } from './job-wizard'
import { MessageFeedback } from "./chat/message-feedback"
import {
  ExpandedChatProvider,
  useExpandedChatContext,
  type TechnicalSkill as ContextTechnicalSkill,
  type BehavioralCompetency as ContextBehavioralCompetency,
  type SalaryInfo as ContextSalaryInfo,
  type WSIQuestion as ContextWSIQuestion,
  type DetectedCriteria as ContextDetectedCriteria,
  type BasicInfoFields as ContextBasicInfoFields,
  type WizardStage,
  type WizardPhase,
...
// Feature flag to enable the new modular wizard
// Set to true to use WizardContainer, false to use the inline wizard code
// NOTE: Modular wizard is still incomplete (missing chat, input, criteria panel)
// Keep as false until WizardContainer is fully implemented
const USE_MODULAR_WIZARD = false

// Skill/Competency interfaces
...
const SKILLS_CATALOG: Record<string, { technical: string[]; behavioral: string[] }> = {
  'engineering': {
...
const ROLE_AREA_MAPPING: Record<string, string> = {
...
// Function to detect area from role
const detectAreaFromRole = (role: string): string | null => {
...
// Function to get skill suggestions based on role and area
const getSkillSuggestions = (role: string | null, area: string | null): { technical: string[]; behavioral: string[] } => {
  let detectedArea: string | null = null
  
  if (role) {
    detectedArea = detectAreaFromRole(role)
  }
  
  if (!detectedArea && area) {
    const areaLower = area.toLowerCase()
    if (areaLower.includes('tecnologia') || areaLower.includes('ti') || areaLower.includes('engenharia')) {
      detectedArea = 'engineering'
    } else if (areaLower.includes('financeiro') || areaLower.includes('contábil') || areaLower.includes('fiscal')) {
      detectedArea = 'finance'
    } else if (areaLower.includes('rh') || areaLower.includes('recursos humanos') || areaLower.includes('people')) {
      detectedArea = 'hr'
    } else if (areaLower.includes('marketing') || areaLower.includes('comunicação')) {
      detectedArea = 'marketing'
    } else if (areaLower.includes('comercial') || areaLower.includes('vendas') || areaLower.includes('sales')) {
      detectedArea = 'sales'
    }
  }
  
  if (detectedArea && SKILLS_CATALOG[detectedArea]) {
    return SKILLS_CATALOG[detectedArea]
  }
  
  return { technical: [], behavioral: [] }
}

// Skill weight inference result interface
interface SkillWeightInference {
  weight: number
  justificativa: string
}

// Core skills mapping for different roles
const CORE_SKILLS_BY_ROLE: Record<string, string[]> = {
...
// Function to detect seniority level from job title
...
// Main inference function for technical skills
const inferTechnicalSkillWeight = (
  ...
  // Check if skill is core for the role
  ...
  if (isCore) {
    // Core skill - high weight with seniority boost
    const baseWeight = 4
    const seniorityBoost = seniorityLevel >= 3 ? 1 : 0
    const finalWeight = Math.min(5, baseWeight + seniorityBoost)
    
    const roleName = cargo || 'esta posição'
    return {
      weight: finalWeight,
      justificativa: `${skill} é competência core para ${roleName}`
    }
  }
  
  // Check if skill is related to the area
  ...
  // Check common tools/technologies
  const commonTools = ['git', 'docker', 'linux', 'sql', 'excel', 'power bi', 'jira']
  if (commonTools.some(tool => skillLower.includes(tool))) {
    return {
      weight: 3,
      justificativa: `${skill} é ferramenta padrão utilizada no mercado`
    }
  }
  
  // Default - desirable skill
  return {
    weight: 2,
    justificativa: `${skill} é competência desejável para complementar o perfil`
  }
}

// Main inference function for behavioral competencies
const inferBehavioralSkillWeight = (
  ...
  // Leadership competencies for leadership roles
  if (isLeadership) {
    if (skillLower.includes('liderança') || skillLower.includes('leadership')) {
      return {
        weight: 5,
        justificativa: `Liderança é essencial para cargos de Gerente/Coordenador`
      }
    }
    if (skillLower.includes('comunicação') || skillLower.includes('communication')) {
      return {
        weight: 5,
        justificativa: `Comunicação é fundamental para gestão de equipes`
      }
    }
    if (skillLower.includes('gestão de pessoas') || skillLower.includes('people management')) {
      return {
        weight: 5,
        justificativa: `Gestão de pessoas é competência core para líderes`
      }
    }
    if (skillLower.includes('visão estratégica') || skillLower.includes('pensamento estratégico')) {
      return {
        weight: 4,
        justificativa: `Pensamento estratégico é esperado de gestores`
      }
    }
  }
  ...
  if (isCommercial) {
    if (skillLower.includes('negociação') || skillLower.includes('negotiation')) {
      return {
        weight: 5,
        justificativa: `Negociação é competência core para área comercial`
      }
    }
    if (skillLower.includes('persuasão') || skillLower.includes('influência')) {
      return {
        weight: 4,
        justificativa: `Persuasão é essencial para conversão de vendas`
      }
    }
    if (skillLower.includes('orientação a resultados') || skillLower.includes('foco em resultados')) {
      return {
        weight: 5,
        justificativa: `Orientação a resultados é fundamental para atingir metas comerciais`
      }
    }
    if (skillLower.includes('resiliência')) {
      return {
        weight: 4,
        justificativa: `Resiliência é importante para lidar com ciclos de vendas`
      }
    }
  }
  ...
  if (isTechnical) {
    if (skillLower.includes('resolução de problemas') || skillLower.includes('problem solving')) {
      return {
        weight: 5,
        justificativa: `Resolução de problemas é competência core para cargos técnicos`
      }
    }
    if (skillLower.includes('pensamento analítico') || skillLower.includes('analytical')) {
      return {
        weight: 5,
        justificativa: `Pensamento analítico é essencial para desenvolvimento de software`
      }
    }
    if (skillLower.includes('aprendizado contínuo') || skillLower.includes('autodesenvolvimento')) {
      return {
        weight: 4,
        justificativa: `Aprendizado contínuo é crucial na área de tecnologia`
      }
    }
  }
  
  // Universal competencies with seniority consideration
  if (skillLower.includes('comunicação') || skillLower.includes('communication')) {
    const weight = seniorityLevel >= 3 ? 5 : 4
    return {
      weight,
      justificativa: `Comunicação é competência essencial para colaboração em equipe`
    }
  }
  ...
  if (skillLower.includes('trabalho em equipe') || skillLower.includes('colaboração')) {
    return {
      weight: 4,
      justificativa: `Trabalho em equipe é fundamental para projetos colaborativos`
    }
  }
  ...
  if (skillLower.includes('adaptabilidade') || skillLower.includes('flexibilidade')) {
    return {
      weight: 3,
      justificativa: `Adaptabilidade é valorizada em ambientes dinâmicos`
    }
  }
  ...
  if (skillLower.includes('proatividade') || skillLower.includes('iniciativa')) {
    ...
    return {
      weight,
      justificativa: `Proatividade é esperada para crescimento profissional`
    }
  }
  ...
  if (skillLower.includes('organização') || skillLower.includes('gestão do tempo')) {
    return {
      weight: 3,
      justificativa: `Organização contribui para produtividade individual`
    }
  }
  ...
  return {
    weight: 3,
    justificativa: `${skill} é competência comportamental relevante para o perfil`
  }
}

// Combined inference function
const inferSkillWeight = (
  skill: string,
  cargo: string,
  senioridade: string,
  area: string,
  ...
  if (type === 'technical') {
    return inferTechnicalSkillWeight(skill, cargo, senioridade, area)
  }
  return inferBehavioralSkillWeight(skill, cargo, senioridade, area)
}
...
interface Message {
  ...
  awaitingStageConfirmation?: string // Stage to advance to if confirmed (conversational flow)
  actionType?: string
...
type WizardMode = 'pre_wizard' | 'create_from_scratch' | 'fast_track' | 'general'
type FastTrackState = 'initial' | 'collecting_criteria' | 'searching' | 'selecting' | 'reviewing' | 'adjusting' | 'publishing' | 'completed'
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Use modular WizardContainer when flag is enabled and in inline job creation mode */}
        {/* TODO: Replace hardcoded companyId with value from auth context when multi-tenant is fully implemented */}
        {inline && isInJobCreationMode && USE_MODULAR_WIZARD ? (
          <WizardContainer 
            onClose={onClose}
