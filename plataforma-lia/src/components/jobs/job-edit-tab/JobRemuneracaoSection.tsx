...
export function JobRemuneracaoSection({
  ...
  // Fallback: estimativa de mercado SO quando nao ha banda pro escopo. So busca
  // depois que o resolve da banda terminou (evita fetch desnecessario).
  const { data: market } = useSalaryBenchmark(title, level, {
    enabled: bandFetched && !hasBand,
  })
  ...
  // Herdado = banda casa + recrutador nao personalizou + (faixa vazia ou == banda).
  const inherited = hasBand && !overriding && (valuesEmpty || eqBand)
      ...
      <div>
        ...
        {inherited && (
          <div className="flex items-center gap-2 mb-2 px-3 py-2 rounded-md border border-lia-border-subtle bg-lia-bg-tertiary/40 text-xs text-lia-text-secondary">
            ...
            <span>
              <strong className="text-lia-text-primary">Herdado da empresa</strong>
              {band ? <> · {bandScopeLabel(band)}</> : null}
              {" — definido em Configurações → Faixas Salariais por Nível."}
            </span>
          </div>
        )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" className={`${inputClass(!isEditing || lockedInputs)} pl-9`} value={minDisplay} onChange={(e) => updateField("salaryMin", e.target.value)} disabled={!isEditing || lockedInputs} placeholder="0,00" />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-lia-text-tertiary">{CURRENCY_SYMBOL}</span>
                  <input type="number" className={`${inputClass(!isEditing || lockedInputs)} pl-9`} value={maxDisplay} onChange={(e) => updateField("salaryMax", e.target.value)} disabled={!isEditing || lockedInputs} placeholder="0,00" />
                </div>
              </div>
            </div>
