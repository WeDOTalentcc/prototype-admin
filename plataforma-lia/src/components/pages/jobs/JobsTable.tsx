...
export function JobsTable({
  ...
  const getStatusBadge = (status: JobVacancy['status']) => {
    const config = {
      ...
      closed: { label: "Encerrada", className: "border-lia-border-default dark:border-lia-border-default text-lia-text-tertiary dark:text-lia-text-tertiary" },
  ...
  const getPriorityBadge = (priority: JobVacancy['priority']) => {
    const config = {
      low: { label: "Baixa", className: "border-lia-border-default dark:border-lia-border-default text-lia-text-tertiary dark:text-lia-text-tertiary" },
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-lia-text-tertiary dark:text-lia-text-tertiary">Carregando vagas...</div>
      </div>
    )
  }
  ...
  if (jobs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-lia-text-tertiary dark:text-lia-text-tertiary">
          ...
          <tr>
            <th className="text-left p-4">
              <button
                ...
                className="flex items-center gap-2 text-sm font-medium text-lia-text-tertiary dark:text-lia-text-tertiary hover:text-lia-text-primary dark:hover:text-lia-text-inverse"
              >
                Vaga
                {getSortIcon("title")}
              </button>
            ...
            <th className="text-right p-4">
              <span className="text-sm font-medium text-lia-text-tertiary dark:text-lia-text-tertiary">Ações</span>
            </th>
          </tr>
        </thead>
        <tbody>
          {jobs.map((job) => (
            ...
            >
              <td className="p-4">
                <div>
                  <div className="font-medium text-lia-text-primary dark:text-lia-text-primary">{job.title}</div>
                  <div className="flex items-center gap-3 mt-1 text-sm text-lia-text-tertiary dark:text-lia-text-tertiary">
                    {job.department && (
              ...
              <td className="p-4 text-center">
                <div className="flex items-center justify-center gap-1">
                  <Users className="h-4 w-4 text-lia-text-disabled dark:text-lia-text-tertiary" />
                  <span className="text-lia-text-primary dark:text-lia-text-primary font-medium">{job.candidatesCount}</span>
                </div>
                {job.applicationsCount > 0 && (
                  <div className="text-xs text-lia-text-disabled dark:text-lia-text-tertiary mt-0.5">
              ...
              <td className="p-4">
                <div className="flex items-center gap-1 text-lia-text-secondary dark:text-lia-text-secondary">
                  <Clock className="h-4 w-4 text-lia-text-disabled dark:text-lia-text-tertiary" />
                  {job.daysOpen || 0} dias
                </div>
              </td>
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-8 w-8 text-lia-text-disabled dark:text-lia-text-tertiary hover:text-lia-text-primary dark:hover:text-lia-text-inverse"
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-8 w-8 text-lia-text-disabled dark:text-lia-text-tertiary hover:text-wedo-orange"
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-8 w-8 text-lia-text-disabled dark:text-lia-text-tertiary hover:text-status-error"
