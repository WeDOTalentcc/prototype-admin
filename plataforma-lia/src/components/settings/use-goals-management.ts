...
import {
  goalTemplates, MONTHS,
  createGoalInBackend, updateGoalInBackend, deleteGoalInBackend, calculateEndDate,
  type GoalTemplate, type UserGoal, type MonthlyGoalValue, type CustomGoalForm, type GoalsUser
} from "./goals-management-types"

export {
  goalTemplates, MONTHS,
  type GoalTemplate, type UserGoal, type MonthlyGoalValue, type CustomGoalForm
} from "./goals-management-types"
...
export function useGoalsManagement(users: GoalsUser[], onGoalUpdate: (userId: string, goals: UserGoal[]) => void) {
  ...
  const setMonthlyValue = useCallback(async (templateId: string, userId: string, month: number, year: number, value: number) => {
    ...
    if (idx >= 0) { updatedValues = [...existing]; updatedValues[idx] = { ...updatedValues[idx], target: value } }
    else { updatedValues = [...existing, { userId, templateId, month, year, target: value, current: 0 }] }
    setMonthlyGoals(prev => ({ ...prev, [key]: updatedValues }))
    try {
      const success = await persistGoalToBackend(templateId, userId, updatedValues)
      if (!success) { setMonthlyGoals(prev => ({ ...prev, [key]: previousValues })); showError('Erro ao salvar meta. Valor revertido.'); return }
      await fetchUserGoals(userId)
    } catch (error) { setMonthlyGoals(prev => ({ ...prev, [key]: previousValues })); showError('Erro ao salvar meta. Valor revertido.') }
  ...
  const applyValueToAll = useCallback(async (templateId: string, value: number, options: { allMonths: boolean, allUsers: boolean }) => {
    ...
    targetUsers.forEach(userId => {
      ...
      const updated = [...(monthlyGoals[key] || [])]
  ...
  const toggleGoalCollapse = (templateId: string) => {
    setCollapsedGoals(prev => { const next = new Set(prev); if (next.has(templateId)) next.delete(templateId); else next.add(templateId); return next })
  }

  const toggleTemplateSelection = (templateId: string) => {
    setSelectedTemplateIds(prev => { const next = new Set(prev); if (next.has(templateId)) next.delete(templateId); else next.add(templateId); return next })
  }

  const handleApplyTemplate = async (template: GoalTemplate, targetUsers: GoalsUser[]) => {
    setIsSaving(true); setSavingTemplateId(template.id); setError(null)
    try {
      ...
      for (const user of targetUsers) {
        ...
        await fetchUserGoals(user.id); applied++
      }
    ...
    } catch (error: unknown) { showError(error instanceof Error ? error.message : 'Erro ao aplicar template') }
    finally { setIsSaving(false); setSavingTemplateId(null) }
  }
  ...
  const handleApplySelectedTemplates = async () => {
    ...
    try {
      ...
      setSelectedTemplateIds(new Set()); setShowTemplates(false)
    } catch (error: unknown) { showError(error instanceof Error ? error.message : 'Erro ao aplicar templates') }
  ...
  const handleCreateCustomGoal = async () => {
    ...
    setIsSaving(true); setError(null)
    try {
      const targetUsersList = isApplyToAll ? users : users.filter(u => u.id === targetUserId)
      ...
      showSuccess(isApplyToAll ? `Meta "${customGoalForm.name}" criada para ${targetUsersList.length} usuários!` : 'Meta criada com sucesso!')
      setShowCustomGoal(false)
      setCustomGoalForm({ name: '', description: '', target: 0, unit: '', period: 'monthly', category: 'recruitment', startDate: '', endDate: '', userId: '' })
      setSelectedUser(null)
    } catch (error: unknown) { showError(error instanceof Error ? error.message : 'Erro ao criar meta') }
    finally { setIsSaving(false) }
  }
  ...
  const handleUpdateGoal = async (goal: UserGoal, updates: Partial<UserGoal>) => {
    ...
    try {
      ...
      await fetchUserGoals(goal.userId); showSuccess('Meta atualizada com sucesso!'); setEditingGoal(null)
    } catch (error: unknown) { showError(error instanceof Error ? error.message : 'Erro ao atualizar meta') }
    finally { setIsSaving(false) }
  }
  ...
  const confirmDeleteGoal = async () => {
    ...
    try {
      ...
      await fetchUserGoals(deleteConfirmGoal.userId); showSuccess('Meta excluída com sucesso!'); setDeleteConfirmGoal(null)
    } catch (error: unknown) { showError(error instanceof Error ? error.message : 'Erro ao excluir meta') }
