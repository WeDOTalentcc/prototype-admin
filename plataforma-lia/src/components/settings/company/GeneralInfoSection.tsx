...
export function GeneralInfoSection({
  ...
  const t = useTranslations("settings.minhaEmpresa.general")
  return (
      ...
      <div className="grid grid-cols-1 gap-3">
        ...
        <div className="inline-flex items-center gap-3 bg-lia-bg-primary dark:bg-lia-bg-secondary border border-lia-border-subtle dark:border-lia-border-subtle rounded-md px-4 py-3 w-fit">
          <div className="relative w-14 h-14 rounded-xl bg-lia-bg-tertiary dark:bg-lia-bg-secondary border-2 border-dashed border-lia-border-default flex items-center justify-center cursor-pointer hover:bg-lia-bg-secondary transition-colors motion-reduce:transition-none hover:border-lia-btn-primary-bg dark:hover:border-lia-border-subtle flex-shrink-0 overflow-hidden">
            {companyData.logo ? (
              <NextImage src={companyData.logo} alt={t("logoAlt")} fill className="object-cover rounded-md" />
            ) : (
              <div className="text-center">
                <Image className="w-5 h-5 mx-auto text-lia-text-tertiary mb-0.5" />
                <span className="text-micro text-lia-text-secondary">{t("logoUpload")}</span>
              </div>
            )}
          </div>
          <div className="flex items-center gap-1.5">
            <span className="text-xs font-medium text-lia-text-primary">{t("logoLabel")}</span>
            <span className="text-micro text-lia-text-secondary bg-lia-bg-tertiary dark:bg-lia-bg-elevated px-1.5 py-0.5 rounded-full">{t("logoCategory")}</span>
          </div>
        </div>

        <SimpleDataCard label={t("legalName")} category={t("registryCategory")} isEditing={isEditing}>
          <input type="text" value={companyData.name} onChange={(e) => setCompanyData((prev) => ({ ...prev, name: e.target.value }))} disabled={!isEditing} className={inputClass(!isEditing)} placeholder={t("legalNamePlaceholder")} />
        </SimpleDataCard>

        <CompanyDataCard fieldKey="trade_name" label={t("tradeName")} category={t("companyInfoCategory")} isActive={companyData.lia_field_toggles?.trade_name ?? true} currentInstruction={companyData.lia_instructions?.trade_name || ""} isEditing={isEditing} onToggleChange={updateLiaToggle} onInstructionSave={updateLiaInstruction}>
          <input type="text" value={companyData.tradeName || ""} onChange={(e) => setCompanyData((prev) => ({ ...prev, tradeName: e.target.value }))} disabled={!isEditing} className={inputClass(!isEditing)} placeholder={t("tradeNamePlaceholder")} />
        </CompanyDataCard>

        <SimpleDataCard label={t("cnpj")} category={t("registryCategory")} isEditing={isEditing}>
          <input type="text" value={companyData.cnpj || ""} onChange={(e) => setCompanyData((prev) => ({ ...prev, cnpj: e.target.value }))} disabled={!isEditing} className={inputClass(!isEditing)} placeholder={t("cnpjPlaceholder")} />
        </SimpleDataCard>

        <CompanyDataCard fieldKey="industry" label={t("industry")} category={t("companyInfoCategory")} isActive={companyData.lia_field_toggles?.industry ?? true} currentInstruction={companyData.lia_instructions?.industry || ""} isEditing={isEditing} onToggleChange={updateLiaToggle} onInstructionSave={updateLiaInstruction}>
