...
export function UnifiedMessageList({
  ...
  return (
    ...
    >
      ...
      {planProgressSteps && planProgressSteps.length > 0 && (
        <div className="group max-w-[90%]">
          <PlanProgressCard
            plan={{
              plan_id: "live",
              status: "in_progress",
              total_tasks: planProgressSteps.length,
              completed: planProgressSteps.filter(s => s.status === "completed").length,
              failed: planProgressSteps.filter(s => s.status === "failed").length,
              skipped: planProgressSteps.filter(s => s.status === "skipped").length,
              pending: planProgressSteps.filter(s => s.status === "pending" || s.status === "running").length,
              tasks: planProgressSteps.map(s => ({
                task_id: s.task_id,
                domain_id: s.domain_id || "automation",
