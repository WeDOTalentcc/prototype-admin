...
export function EditCriteriaModal({ open, criteria, onClose, onAddCriterion, onRemoveCriterion }: EditCriteriaModalProps) {
 ...
 "text-xs px-2 py-0.5 rounded-md",
                    criterion.source === 'technical' ? 'bg-gray-100 dark:bg-lia-bg-secondary text-gray-600 dark:text-lia-text-tertiary' : 'bg-wedo-purple/10 text-wedo-purple'
                  )}>
                    {criterion.source === 'technical' ? 'Técnico' : 'Comportamental'}
                  </span>
                  <button
                    onClick={() => onRemoveCriterion(criterion.id)}
                    className="p-1.5 rounded-md lia-text-muted hover:text-status-error hover:bg-status-error/10 transition-colors opacity-0 group-hover:opacity-100"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
            ...
            <div className="px-6 py-4 border-t border-lia-border-subtle space-y-3">
              <div className="flex gap-2">
                <button className="flex items-center gap-2 px-4 py-2 text-sm lia-text-secondary hover:lia-text-strong transition-colors">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-2.5 px-4 border border-lia-border-subtle rounded-md text-sm font-medium lia-text-secondary hover:border-gray-900 dark:hover:border-gray-50 hover:lia-text-strong dark:hover:lia-text-subtle transition-colors flex items-center justify-center gap-2"
                >
                ...
                <button
                  onClick={onClose}
                  className="flex-1 py-2.5 px-4 bg-gray-900 text-white rounded-md text-sm font-medium hover:bg-gray-800 dark:hover:bg-gray-200 transition-colors"
                >
                  Atualizar ↗
                </button>
              </div>
