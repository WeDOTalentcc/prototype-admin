      ...
      <div className="p-3 rounded-md space-y-2 border border-gray-200 bg-gray-50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-gray-800 dark:text-gray-100">Perfil Ideal sugerido pela LIA</span>
          </div>
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger><HelpCircle className="w-3.5 h-3.5 text-gray-600" /></TooltipTrigger>
        ...
        <div className="flex flex-wrap gap-1.5">
          {combinedSuggestions.map((keyword) => (
            <div key={keyword} className="flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium group border border-gray-200 bg-white">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button type="button" onClick={handleSubmit} disabled={!canSubmit() || isLoading}
                        className={cn("flex items-center justify-center p-1.5 rounded-md transition-all", canSubmit() ? "hover:bg-gray-100 text-gray-400" : "opacity-50 cursor-not-allowed text-gray-200")}>
...
function ScopeButton({ icon: Icon, active, activeColor, onClick, label, sublabel, small }: {
  ...
  const textClass = activeColor === 'wedo-green' ? 'text-wedo-green'
    ...
    : activeColor === 'wedo-cyan' ? 'text-gray-950'
    : ''
  return (
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); onClick() }}
            className={cn("flex items-center justify-center p-1.5 rounded-md text-xs transition-all", active ? ringClass : "hover:bg-gray-100", active ? textClass : "text-gray-400")}>
            <Icon className={size} />
          </button>
        </TooltipTrigger>
        <TooltipContent side="bottom" className="!animate-none !duration-0">
