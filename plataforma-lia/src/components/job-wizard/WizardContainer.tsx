...
function WizardContent({ onClose, onMinimize, onJobCreated, inline }: Omit<WizardContainerProps, 'companyId'>) {
 ...
 "w-4 h-4 rounded-full flex items-center justify-center text-micro font-semibold",
          ...
          !isCurrent && !isPast && "border border-lia-border-default text-lia-text-secondary"
        )}>
 ...
 "text-micro font-medium px-2 py-0.5 rounded-full",
                ...
                {phaseIndex < WIZARD_PHASES.length - 1 && (
                  <ChevronRight className="w-3 h-3 text-lia-text-secondary" />
                )}
              </React.Fragment>
            ))}
          </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-col transition-colors motion-reduce:transition-none duration-300 flex-1" style={{width: isPanelOpen ? `${100 - panelWidth}%` : '100%'}}>
          <div className="flex-1 overflow-y-auto p-4">
            ...
            <div className="text-center text-lia-text-secondary py-8">
              <p className="text-sm">Stage: {currentStageConfig?.title}</p>
              <p className="text-xs text-lia-text-secondary mt-1">{currentStageConfig?.liaMessage?.slice(0, 100)}...</p>
            </div>
          </div>
        </div>
 ...
 "flex items-center gap-1.5 px-4 py-2 rounded-md text-xs font-medium transition-colors",
              canAdvance || currentStage === 'salary'
                ...
                : "bg-lia-border-default text-lia-text-secondary cursor-not-allowed"
            )}
