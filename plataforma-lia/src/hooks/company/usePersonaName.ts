 ...
 * Configurações › Personalidade da IA (persona per-tenant). Default "IA".
 *
 ...
 * do assistente é exibido ao usuário, em vez de hardcodar "IA" (white-label).
 */
import { useAiPersona } from "@/hooks/company/use-ai-persona"

export function usePersonaName(): string {
  const { persona } = useAiPersona()
  return persona?.name?.trim() || "IA"
}
