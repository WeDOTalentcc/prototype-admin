...
import {
  Phone, Target, CheckCircle, Calendar, Award, AlertCircle
} from "lucide-react"
...
export const generateTimeSeriesData = (currentValue: number, volatility: number = 5) => {
...
export function useMetricsCalculations(candidates: DashboardCandidate[]) {
  ...
  const sourceMetrics: SourceMetricItem[] = (Object.values(sourceMetricsObj) as SourceMetric[]).map((source) => ({
    ...source,
    conversionRate: source.count > 0 ? (source.contacted / source.count) * 100 : 0
  }))

  const candidateStatusBreakdown: StatusBreakdownItem[] = [
    { status: 'Contato Inicial', count: contacted, percentage: (contacted / totalCandidates) * 100, icon: React.createElement(Phone, { className: "w-4 h-4" }) },
    { status: 'Triagem Iniciada', count: triageStarted, percentage: (triageStarted / totalCandidates) * 100, icon: React.createElement(Target, { className: "w-4 h-4" }) },
    { status: 'Triagem Completa', count: triageCompleted, percentage: (triageCompleted / totalCandidates) * 100, icon: React.createElement(CheckCircle, { className: "w-4 h-4" }) },
    { status: 'Triagem Aprovada', count: triageApproved, percentage: (triageApproved / totalCandidates) * 100, icon: React.createElement(Award, { className: "w-4 h-4" }) },
    { status: 'Entrevista Agendada', count: interviewScheduled, percentage: (interviewScheduled / totalCandidates) * 100, icon: React.createElement(Calendar, { className: "w-4 h-4" }) },
    { status: 'Reprovado', count: candidates.filter(c => c.status === 'Reprovado').length, percentage: (candidates.filter(c => c.status === 'Reprovado').length / totalCandidates) * 100, icon: React.createElement(AlertCircle, { className: "w-4 h-4" }) },
  ]

  return {
    totalCandidates, contacted, contactRate,
    ...
    pendingApproval,
    stageBreakdown, funnelStages, scoreDistribution,
