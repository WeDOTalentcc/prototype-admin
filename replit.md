...
- **Choose Your AI — LLM Config (Task #94)**: Tenant-level LLM provider management. Backend: `llm_config_router` at `/api/v1/admin/llm-config` (GET/PUT/POST test/GET providers). DB: `tenant_llm_configs` table with encrypted API key storage via Fernet. Frontend: `ApiKeyConfigForm` component in IntegrationDetailDrawer with key validation (calls `/test` endpoint before saving), masking of saved keys, and real-time status updates in the integrations hub. Proxy routes: `/api/backend-proxy/llm-config` (GET+PUT), `/test` (POST), `/providers` (GET).
