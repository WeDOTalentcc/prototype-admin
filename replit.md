...
- **TipTap Rich Text Editor Integration**: Replaced raw HTML textarea in email templates and plain textarea in job description editing with TipTap WYSIWYG editor. Components: `<LiaEditor>` base component (`plataforma-lia/src/components/ui/lia-editor.tsx`) with configurable toolbar (full/basic/none), dark mode support, and DS token integration. Custom `TemplateVariable` TipTap extension (`lia-editor-variable-extension.ts`) renders `{{variable}}` as non-editable visual chips. Email template form uses full toolbar + variable chips. Job description editor uses basic toolbar (headings, lists, bold). Dependencies: @tiptap/react, @tiptap/starter-kit, @tiptap/extension-link, @tiptap/extension-placeholder, @tiptap/pm.
