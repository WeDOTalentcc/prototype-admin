...
- **Task #110 — Design System + Accessibility + Dead Code**: Fixed `text-micro` on Badge components in `candidate-preview.tsx` (replaced with `text-[0.625rem] leading-none` to avoid CVA `text-xs` conflict). Removed 5 `@ts-ignore` from `chart-components.tsx` and `success-prediction.tsx`. Deleted dead code: `chart-components.tsx` (0 imports), `success-prediction.tsx` (0 imports), empty `ml-analytics/` dir. Added `aria-label` to icon-only buttons (stage-transition-actions-modal, PortalFieldRenderer). Verified all `<img>` and `<Image>` tags already have alt text. Error boundary + error.tsx + not-found.tsx already exist at root level. 0 TypeScript errors.
