...
- **Multi-Tenancy `company_id` Isolation**: Added `client_account_id` FK to `CompanyProfile` model (column added to DB), created tenant resolution endpoint, built centralized `useCompanyId` React hook, and replaced hardcoded `company_id`. The `useCurrentCompany` hook exposes both `companyId` (profile UUID) and `tenantId` (user's company_id from auth store, e.g. "demo_company"). User management operations use `tenantId` for correct user filtering. The `get_company_profile` endpoint falls back gracefully: tries `client_account_id` match → UUID match → default profile.
