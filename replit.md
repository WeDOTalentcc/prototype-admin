...
- **Per-Request Cost Tracking & Budget Alerts**: `TenantBudget` extended with per-request cost recording via Redis hash keys (`req_cost:{company_id}:{request_id}`). `check_and_record()` now accepts `request_id`, `tokens_input/output`, `cost_usd`, and `model` for granular billing. Per-request token threshold alert at `TENANT_PER_REQUEST_TOKEN_THRESHOLD` (default: 50k). New `RequestCostRecord` dataclass in audit models. `ExecutionAuditRecord` extended with `request_id` and `request_cost` fields. Audit writer persists request cost to PG with graceful fallback.
- **Recursive RAG Chunking**: New `RecursiveTextSplitter` strategy in `app/shared/intelligence/chunking/recursive.py` — hierarchical splitting by semantic boundaries (paragraphs → sentences → clauses → words → chars). Registered as "recursive" in `ChunkingStrategyFactory`. Per-doc-type chunk size/overlap configurable via settings (`CHUNKING_CV_CHUNK_SIZE`, `CHUNKING_JD_CHUNK_SIZE`, etc.). Factory `get_strategy()` accepts explicit `chunk_size`/`chunk_overlap` kwargs.
- **Monetizable Modules Infrastructure**: `CompanyModule` model in `lia_models/billing.py` with `company_modules` table (migration 066). `ModuleService` in `app/domains/modules/services/module_service.py` with methods: `is_module_active()`, `get_company_modules()`, `activate_module()`, `update_module()`, `deactivate_module()`, `get_module_tier()`, `seed_beta_modules()`. REST API at `/api/v1/modules/` with tenant isolation (`_enforce_tenant`). 7 registered modules: 5 BETA (`talent_intelligence_pro`, `internal_mobility`, `interview_intelligence`, `workforce_planning`, `candidate_nurture`) + 2 coming_soon (`onboarding_suite`, `predictive_analytics`). Module statuses: beta/trial/active/expired/disabled/coming_soon. Tiers: free/basic/pro/enterprise. `is_module_active()` returns false for explicitly disabled/expired modules (no feature-flag fallback override), falls back to `FeatureFlagService` only when no module record exists. Migration seeds BETA modules for existing credit_accounts. 46 unit tests.
- **Module-Aware Tool Gating (Task #158)**: `app/shared/module_gating.py` provides fail-closed access control for premium tools. `TOOL_MODULE_MAP` maps 11 tools → 5 modules. `PREMIUM_GATED_TOOLS` (3 tools: fully blocked when module inactive) and `TASTING_TOOLS` (8 tools: return partial preview + CTA when inactive). `tool_handler` decorator extended with `module` parameter — performs single access check, returns degraded response with module CTA when denied, adds BETA badge when module is in beta status. Security: fail-closed on missing context (no `db`/`company_id`), fail-closed on service errors, no feature-flag bypass. `check_tool_module_access()` and `build_degraded_response()` for integration. `_extract_tasting_data()` extracts partial previews (skills, candidates, gaps, metrics). All 11 talent intelligence tools decorated with module gating. 39 unit tests in `test_module_gating.py`.
