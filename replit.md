...
- **Wizard de criação de vaga** — `WizardReActAgent` herda de `TenantAwareAgentMixin` com `tenant_strict_override = True` (NUNCA degrada para `"sua empresa"/"geral"`, mesmo com `LIA_AGENT_TENANT_STRICT=false` em dev). Detalhes em [`docs/architecture/tenant-context-history.md`](./docs/architecture/tenant-context-history.md) §T-B. **Continuidade de sessão (Task #1080 — refactor canônico):** o `thread_id` do wizard é derivado por uma ÚNICA função pura `app.shared.sessions.derive_thread_id(company_id, session_id) -> "wiz-{token8}-{session_id}"` — sem honor de `msg["thread_id"]` custom, sem Redis marker (`lia:wizard:active:*` foi extinto), sem heurística `_candidate_thread_ids`, sem Tier 0.5 no `CascadedRouter`. "Esta sessão é wizard" é detectado por `app.shared.sessions.is_wizard_session_active(company_id, session_id)`, que lê o checkpoint LangGraph para esse único `thread_id` e retorna True iff existe estado E `current_stage != "completed"` (fail-open em outage). O **pin** vive nos handlers de transport (`agent_chat_ws.py` e `agent_chat_sse.py`) ANTES da chamada ao `CascadedRouter` — quando o FE não enviou um `domain` explícito (`active_domain in ("auto", "recruiter_assistant", "")`) e a sessão tem checkpoint aberto, força `active_domain = "wizard"` e pula o roteamento normal. O `main_orchestrator` (REST) já operava com a mesma checagem via `WizardSessionService.is_session_active` (mantida como wrapper delegante). Esse modelo elimina o bug do `pw-cenario-D2` (`page.reload()` mid-wizard perdia o `thread_id` custom em memória do FE e expirava o marker Redis → router reclassificava, checkpointer não restaurava, B2/B3/B4 voltavam): `derive_thread_id` é pura, então `(company_id, session_id)` reproduz o MESMO `thread_id` após reload, e o pin nos handlers consulta o checkpoint diretamente. `WizardSessionService.derive_thread_id` e `.is_session_active` permanecem como wrappers delegantes finos (1 linha cada) ao canônico para preservar mocks de testes externos. Sentinela: `tests/integration/agents/test_wizard_session_continuity_t1080.py` (S1 pureza/determinismo, S2 anti-honor msg.thread_id, S3 fail-open, S4 AST router-clean, S5 wrappers delegam, S6 handlers donos do pin). E2E: `plataforma-lia/e2e/tests/wizard/10-session-continuity.spec.ts` (loop reload mid-wizard + anti-pergunta de empresa). **Eval gate online (Task #1052):** `eval/golden/wizard_no_tenant_leak.jsonl` reproduz, em conversação ao vivo contra o backend, os 4 bugs originais — B1 (NÃO pergunta `company_id` em texto livre), B2 (mantém o título da vaga entre turnos), B3 (NÃO afirma ter perdido o histórico), B4 (a salary tool NÃO re-pergunta empresa/setor) — em 12 cenários: 3 single-turn (B1) + 9 multi-turn de 3 turnos (B2/B3/B4 × 3 vagas: Engenheiro Backend, Analista Financeiro, Coordenador de Marketing). Cada caso multi-turn replay é executado sobre uma ÚNICA `conversation_id` (capturada da primeira resposta e devolvida nos turnos seguintes), forçando o checkpointer LangGraph + o pin handler-level do wizard (Task #1080) a participarem do gate — sem isso, B2/B3/B4 não seriam observáveis. Em CI, rodar primeiro a execução ao vivo e depois o gate (espelha T-E e CSP, threshold 0.85): `python -m eval.eval_runner --dataset eval/golden/wizard_no_tenant_leak.jsonl && python -m eval.eval_runner --gate eval/golden/wizard_no_tenant_leak.jsonl`. O scorer aplica `re.search` (IGNORECASE/DOTALL) sobre os `anti_patterns` regex e exige pelo menos um `expected_snippet_markers` (ex.: "Demo Company", "Backend") na resposta final como evidência de que o `tenant_context_snippet` foi honrado. Requer `LIA_TEST_TOKEN` válido + backend rodando — runner usa o JWT auto-gerado de `eval_runner.py:_make_eval_token`. Helper paralelo `_heal_legacy_demo_company_id` em `app/auth/dependencies.py` reconcilia in-place QUALQUER `company_id` demo não-canônico (legacy `"demo_company"` ou outras strings inválidas) para `CANONICAL_DEMO_UUID` antes de `CompanyId.parse` consumir (B1 fix; comportamento amplo é intencional — qualquer estado inválido em user demo é hygiene).
- **Wizard resume via interrupt (Task #1118)** — qualquer PR que toque os 4 nós com `interrupt()` canônico (`jd_gate_node`, `competency_gate_node`, `wsi_questions_gate_node`, `review_gate_node` em `lia-agent-system/app/domains/job_creation/graph.py`) ou `WizardGateService._resume_engine` deve rodar a spec E2E `plataforma-lia/e2e/tests/wizard/14-resume-via-interrupt.spec.ts` (cobre os 4 gates aprovando 3× via chat livre + 1× via botão em uma única `conversation_id`, asserta zero repetição de pergunta + EXATAMENTE 4 audit rows `wizard_gate_service/resume_gate`) e o eval gate `eval/golden/wizard_resume_via_interrupt.jsonl` (4 cenários multi-turn que disparam `_resume_engine` ao vivo via classifier LLM, threshold 0.85). CI: `bash plataforma-lia/scripts/run-pw-cenario.sh pw-cenario-1118 e2e/tests/wizard/14-resume-via-interrupt.spec.ts` e `python -m eval.eval_runner --dataset eval/golden/wizard_resume_via_interrupt.jsonl && python -m eval.eval_runner --gate eval/golden/wizard_resume_via_interrupt.jsonl`. Requer `LIA_WIZARD_LLM_GATES=true` e `LIA_SECRET_KEY` no env.
...
| `LIA_WIZARD_LLM_GATES=true` | **(T2 / Task #1085)** Liga o gate LLM-based em `jd_enrichment` (HITL #1) — substitui o classifier brittle keyword-based de `domain.py::_route_by_stage` por classificador Pydantic-validado (Claude Haiku, temp=0, allowlist `approve | reject_with_feedback | provide_new_content | ask_question | off_topic`). Default: ON em dev/test, OFF em prod/staging (preserva `route_after_jd` legacy até GA + migração de gates competency/wsi/review nas Tasks #4/#5/#6). Timeout: `LIA_WIZARD_GATE_CLASSIFIER_TIMEOUT_S` (default 5s). Modelo: `LIA_WIZARD_GATE_CLASSIFIER_MODEL` (default `claude-3-5-haiku-20241022`). Resolve bug "wizard repete `preciso aprovação` 4× ignorando `manda bala`/`tá liberado`/`fica bom`". Sentinela: `tests/integration/agents/test_wizard_gate_engine_t2.py`. Eval gate: `eval/golden/wizard_conversational_hitl.jsonl` (5 cenários canônicos). E2E: `plataforma-lia/e2e/tests/wizard/11-conversational-hitl.spec.ts`. Mutação de state é **determinística** (mapeia `intent` ∈ allowlist → fields fixos); `conversational_reply` do LLM nunca é executado nem usado como controle de fluxo. FairnessGuard L1 roda sobre a mensagem de resume ANTES do classifier. Audit row por gate (`decision_type=wizard_step_completed`, `agent_name=wizard_jd_gate_classifier`). |
