...
- **Job Creation & Candidate Management**: Supports conversational and manual job creation, with a Kanban candidate movement system featuring AI-powered sub-status prediction and WSI score integration. The legacy Job Readiness Hub was deprecated and removed in Task #791 in favor of the unified funnel/Kanban view; ORM no longer maps `readiness_stage`, `readiness_blockers`, `last_readiness_event_at`, or `assigned_audience_policy` (DB columns from migration 086 retained for historical data).
