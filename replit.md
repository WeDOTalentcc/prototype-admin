...
- **Task #729 — Reconciliar schema drift de `recruitment_campaigns` (caso histórico de `Base.metadata.create_all`)**: o endpoint `/api/v1/recruitment_campaigns` quebrou em dev com `UndefinedColumnError: column recruitment_campaigns.created_by does not exist`, mesmo com Alembic em head `096_*`. Investigação pelo protocolo `canonical-fix` mostrou que a tabela tinha sido criada por `Base.metadata.create_all` (em `app/core/database.py:init_db` e `scripts/seed_data.py:main`) num boot anterior, antes da migration 064 existir, e desde então NUNCA foi reconciliada — faltavam 11 colunas (`created_by`, `description`, `stages`, `current_stage_index`, `total_candidates`, `candidates_screened`, `candidates_contacted`, `candidates_interviewed`, `candidates_offered`, `candidates_hired`, `stage_history`) e os 3 índices (`idx_campaign_company/status/job`). Em paralelo a tabela tinha duas colunas órfãs (`current_stage varchar`, `stages_config jsonb`) que o boot antigo emitia mas o modelo atual não declara — preservadas (sem dataloss; remoção fica para depois que `create_all` for retirado). Migração nova `097_reconcile_recruitment_campaigns_columns` é idempotente: lê `inspector.get_columns(...)`, faz `ADD COLUMN ... DEFAULT 'system'/0/'[]'` para cada coluna ausente (PostgreSQL backfila linhas existentes na mesma ALTER TABLE), depois `UPDATE` defensivo por NULLs vindos de migrações parciais, então `ALTER COLUMN ... SET NOT NULL` em `created_by`/`stages`/`current_stage_index`/`stage_history`, e cria os 3 índices se faltarem. Aplicada em dev (`alembic upgrade head` ✓; `\d recruitment_campaigns` confirma 22 colunas + 3 índices); SELECT do modelo via SQLAlchemy retorna 0 rows sem erro. Anti-padrão #7 do `canonical-fix` (migration inline / schema fora do Alembic) marcado como dívida técnica nos dois call-sites de `Base.metadata.create_all` com plano de remoção (gate `ALEMBIC_AUTO_CREATE=false` numa task de follow-up). Testes de regressão em `tests/integration/test_recruitment_campaigns_schema.py` (2 PASSED): (a) DB tem todas as 20 colunas requeridas, (b) modelo declara todas as 20 colunas — qualquer regressão futura quebra o teste antes de chegar em runtime. Schema drift histórico documentado para a próxima vez que aparecer um caso parecido: o sintoma é "endpoint 500 com `UndefinedColumnError` mesmo com Alembic em head", e o caminho de fix é (1) `\d <tabela>` + comparar com modelo, (2) `grep Base.metadata.create_all` para localizar o boot path culpado, (3) migration idempotente nova com `ADD COLUMN IF NOT EXISTS` via inspector, (4) marcar o `create_all` como deprecated com plano de remoção.
