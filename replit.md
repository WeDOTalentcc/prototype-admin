...
-   **Roadmap MVP Alpha 1 — Cards Unificados v5.0**: Documento `docs/roadmap-mvp-alpha1-cards-unificados.md` (~5.900 linhas). v5.0 adiciona 15 seções globais de referência IA: Tools Registry (91 tools Alpha 1 por agente), NFRs (latência P50/P95, disponibilidade, rate limits por tenant/plano), Env vars por épico, LLM Cascade (Haiku→Sonnet→Opus + fallbacks), HITL map (14 ações com/sem confirmação humana), Prompt templates (9 YAMLs de domínio + prompt registry), Limites operacionais (12 recursos com limites), Anti-patterns (8 regras NUNCA), Migrations Alembic (10 migrations mínimas), Production templates (11 templates de comunicação), Webhooks (8 pontos de integração externa), Checklist de impacto feature-impact (12 dimensões), Checklist de conformidade IA (18 itens obrigatórios por agente), Shared tools (8 tools cross-agent), Infraestrutura compartilhada (12 componentes obrigatórios). Fontes cruzadas: `diagnostico-agentes-mvp.md` (§0B, §8, §13, §13B, §14, §20, §21) + `feature-impact` skill. Total: 44 cards · 222 SPs.
