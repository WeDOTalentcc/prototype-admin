...
## React Composition Patterns (from vercel-composition-patterns)

Composition patterns for flexible, maintainable React components. Avoid boolean prop proliferation by using compound components, lifting state, and composing internals.
...
**Wrong:**
```tsx
function Composer({ isThread, isEditing, isForwarding, isDMThread }: Props) {
  return (
    <form>
...
**Right — explicit variant components:**
```tsx
...
### Rule 2: Compound Components with Shared Context (HIGH)
...
function ComposerInput() {
  ...
  return <textarea value={state.text} onChange={actions.setText} />
}
...
const Composer = { Provider: ComposerProvider, Input: ComposerInput, Submit: ComposerSubmit }
```

### Rule 3: Decouple State from UI (MEDIUM)

Provider is the only place that knows how state is managed. Components consume via context interface.

```tsx
interface ComposerContextValue {
  state: { text: string; attachments: File[] }
  actions: { setText: (t: string) => void; submit: () => void }
  meta: { isSubmitting: boolean; charCount: number }
}
```

This makes swapping state management (useState -> Zustand -> Redux) a single-file change.

### Rule 4: Lift State into Providers (MEDIUM)

Move state up into provider components when siblings need access.
...
function ComposerProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <ComposerContext value={{
      ...
      actions: { setText, submit: () => { /* ... */ } },
      meta: { isSubmitting: false, charCount }
    }}>
      {children}
    </ComposerContext>
  )
...
### Rule 5: Prefer Children Over Render Props (MEDIUM)

```tsx
// Wrong
<Composer renderFooter={() => <Footer />} renderHeader={() => <Header />} />
...
### Rule 6: React 19 API Changes

- Do NOT use `forwardRef` — pass `ref` as a regular prop
...
function ComposerInput() {
  ...
  return <textarea value={ctx.state.text} />
}
```
...
**Skill Version**: 2.0.0
**Pattern Coverage**: 23 GoF patterns + React Composition Patterns
