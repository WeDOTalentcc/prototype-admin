...
- **Cadeia de enriquecimento de email (`require_emails`):** (1) Pearch retorna perfis com `has_email=True` (sabe que existe, valor mascarado); (2) `enrich_and_filter_candidates` revela o valor real via Apify — `enrich_batch` para candidatos com UUID no banco, `enrich_by_linkedin_url` (Apify direto) para perfis novos com URL LinkedIn (`_shared.py:670`); (3) rede de segurança final descarta quem ainda não tem `has_email` nem `email` após o enriquecimento (`search.py:396-402`). **Apify não é "fallback" da Pearch — ela revela o valor.** Se a Apify não consegue revelar, o candidato pode permanecer com email mascarado e ser revelado sob demanda (botão "Revelar").
- **O mesmo filtro se aplica ao load-more/refine** (`POST /candidates/refine` — `jd_search.py:213-219`): o endpoint recebe `require_emails` e aplica a mesma rede de segurança final no incremento de resultados. O frontend já propaga a flag na chamada de refine.
...
- **TTL LGPD 180 dias (`external_candidate_profiles`):** perfis Pearch ficam em `external_candidate_profiles` apenas enquanto relevantes. O `lgpd_cleanup_service.py` deleta perfis com status diferente de *promoted* após 180 dias sem engajamento (`RETENTION_DAYS["pearch_discovered_profiles"] = 180` — LGPD Art. 5º, minimização). Isso é distinto do suppression de sessão (500 docids): o suppression evita re-pagar na mesma sessão; o TTL remove da base após 6 meses de inatividade.
...
- **Aviso ao sair sem salvar resultados Pearch (`UnsavedPearchWarningModal`):** ao tentar iniciar nova busca ou navegar com candidatos Pearch não salvos na base, o `NavGuardStore` intercepta e abre o modal (`candidates-page.tsx:191-192`, `nav-guard-store.ts`). O modal mostra quantos candidatos têm email/telefone, créditos consumidos, e oferece: **"Salvar e Sair"** (`POST /candidates/persist-revealed`) ou **"Sair sem Salvar"** (perde os contatos revelados mas já pagos). Quem replicar deve implementar a mesma guarda ou alertar que créditos são perdidos.
...
- **Refine/load-more (endpoint `/candidates/refine`):** aplica o mesmo `require_emails` loop + filtro final que a busca inicial. O `thread_id` mantém o contexto de paginação Pearch entre chamadas (`jd_search.py:159`). Ver §4.6c para a cadeia completa.
