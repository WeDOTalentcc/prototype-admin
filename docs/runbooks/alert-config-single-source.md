...
| SLA Próximo do Vencimento | MonitoringLoop `_check_sla_risks` | — | `sla_near_expiration` | on / 80% / 12h | email, bell, teams |
...
as tabelas legadas ficam órfãs por design (sem migração nesta task). Criar
detectores para as regras órfãs do catálogo é tarefa seguinte.

## Sentinelas

`tests/contract/test_alert_config_single_source.py` (10 testes):
