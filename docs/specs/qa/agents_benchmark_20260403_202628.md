...
| Aprovados | 7 |
| Reprovados | 6 |
| Score médio | 71.9/100 |
| Latência média | 12077ms |
...
|----|------|--------|-------|----------|--------|
| wsi_001 | WSI — Resposta STAR completa (deve score >= 7) | ❌ HTTP 200 | 60/100 | 15295.7ms | Score 3.8 fora do intervalo esperado [7.0, 10.0] |
| wsi_002 | WSI — Resposta vaga (deve score <= 4) | ✅ HTTP 200 | 85/100 | 9413.1ms | — |
| wsi_003 | WSI — Resposta parcial STAR (deve score 4-7) | ❌ HTTP 200 | 45/100 | 8676.1ms | Score 1.5 fora do intervalo esperado [4.0, 7.0] |
| wsi_004 | WSI — Resposta irrelevante/off-topic (deve score <= 3) | ✅ HTTP 200 | 85/100 | 8541.0ms | — |

### Agente: `CV_MATCH`

| ID | Nome | Status | Score | Latência | Issues |
|----|------|--------|-------|----------|--------|
| cv_001 | CV Match — Match forte (deve match_score > 80) | ❌ HTTP 200 | 70/100 | 15205.6ms | Lista 'matched_skills' ausente ou vazia |
| cv_002 | CV Match — Match fraco (deve match_score < 30) | ❌ HTTP 200 | 70/100 | 15235.2ms | Lista 'missing_skills' ausente ou vazia |
| cv_003 | CV Match — Match parcial (deve match_score 40-70) | ❌ HTTP 200 | 30/100 | 13945.5ms | match_score 28.0 fora do intervalo esperado [40.0, 70.0] |

### Agente: `SALARY`

| ID | Nome | Status | Score | Latência | Issues |
|----|------|--------|-------|----------|--------|
| salary_001 | Salary — Engenheiro de Dados Sênior SP (R$ 12k-25k) | ✅ HTTP 200 | 90/100 | 17522.4ms | — |
| salary_002 | Salary — Analista de Dados Júnior BH (R$ 3k-6k) | ✅ HTTP 200 | 100/100 | 12217.9ms | — |

### Agente: `PIPELINE`

| ID | Nome | Status | Score | Latência | Issues |
|----|------|--------|-------|----------|--------|
| pipeline_001 | Pipeline — Saúde geral do funil | ✅ HTTP 200 | 100/100 | 14239.4ms | — |
| pipeline_002 | Pipeline — Tempo médio de contratação tech | ❌ HTTP 0 | 0/100 | 0ms | Falha de conexão:  |

### Agente: `CANDIDATE_SEARCH`

| ID | Nome | Status | Score | Latência | Issues |
|----|------|--------|-------|----------|--------|
| search_001 | Candidate Search — Engenheiro Python Sênior | ✅ HTTP 200 | 100/100 | 15259.3ms | — |
| search_002 | Candidate Search — Product Manager | ✅ HTTP 200 | 100/100 | 11454.1ms | — |
...
████████████████████░░  92% Match
```
### ✅ Resultado: **ALTAMENTE RECOMENDADO para avanço**
...
> # ❌ Match: ~8% — Incompatível

---

## 🔍 Análise Detalhada por Critério

| Critério | Requisito da Vaga | C
  ```
...
| Critério | Requisito | Candidato | Status |
|---|---|---|---|
| **Python**
  ```
