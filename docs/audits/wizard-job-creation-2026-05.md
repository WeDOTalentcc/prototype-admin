...
| **D3** | Média | Audit row apenas em `jd_enrichment` (FG L3 block) e `publish`. Nodes intermediários (bigfive, wsi_questions) não emitem audit explícito quando completam OK. EU AI Act sugere audit por step decisório. | Adicionar `audit_storage` row em `bigfive`, `wsi_questions`, `competency`, `eligibility` quando completam (decision="completed"), espelhando padrão do `audit_company_change` em `company_settings`. | Task #1059 ou nova. |
